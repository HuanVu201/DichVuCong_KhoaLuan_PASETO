...
import { EXPAND_OFFICE_NAME, MA_PHIEU_YEU_CAU_BO_SUNG_HO_SO_TRUC_TUYEN, TEN_BO_PHAN, TEN_PHIEU_YEU_CAU_BO_SUNG_HO_SO_TRUC_TUYEN, TEN_TRUNG_TAM } from ".."
...
export const PhieuYeuCauBoSungTrucTuyen = ({ lyDoBoSung, setUrlPhieu, maGiayToHoSo, setMaGiayToHoSo, pdfBlob, setPdfBlob, xuatWord, setXuatWord, onChangeLyDo, loadingPhieu, setLoadingPhieu }:
    {
        lyDoBoSung: string | undefined, setUrlPhieu: (url: string) => void,
        ...
        (async () => {
            ...
            if (buttonActionContext.selectedHoSos.length && !hoSo) {
                const res = await hoSoApi.XuatPhieuAction({
                    ...
                    code: 'phieu-yeu-cau-bo-sung-ho-so-truc-tuyen',
                    tenGiayTo: 'Phiếu yêu cầu bổ sung hồ sơ nộp trực tuyến',
                    maLoaiPhieu: `${MA_PHIEU_YEU_CAU_BO_SUNG_HO_SO_TRUC_TUYEN}`,
    ...
    useEffect(() => {
        if (hoSo) {
            setMaGiayToHoSo(`${hoSo.maHoSo ? hoSo.maHoSo : hoSo.id}_${MA_PHIEU_YEU_CAU_BO_SUNG_HO_SO_TRUC_TUYEN}`)
    ...
    useEffect(() => {
        (async () => {
            ...
            if (maGiayToHoSo && lyDoBoSung && !xuatWord) {
                ...
                }, "PhieuYeuCauBoSungTrucTuyen.phoi.docx", async (blob) => {
