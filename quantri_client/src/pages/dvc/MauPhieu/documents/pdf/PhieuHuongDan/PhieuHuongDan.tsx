import { API_VERSION, CURRENTTIME_ISOSTRING, HOST_PATH, UPLOADFILE_ENDPOINT } from "@/data"
...
export const PhieuHuongDan = () => {
    ...
    useEffect(() => {
        if (huongDanNopHoSo) {
            xuLyHoSoContext.setMaGiayToHoSo(`${huongDanNopHoSo?.maHoSo ? huongDanNopHoSo.maHoSo : huongDanNopHoSo?.id}_${MA_PHIEU_HUONG_DAN_HOAN_THIEN_HO_SO}`)
        }
        ...
        (async () => {
            ...
            if (xuLyHoSoContext.qrThongTinPhieu && !xuLyHoSoContext.xuatWord) {
                ...
                await generateDocxWithImages(huongDanNopHoSo?.urlPhoi || '', {
                    ...
                    thanhPhanHoSos,
                    nguoiHuongDan: huongDanNopHoSo?.nguoiTiepNhan ? huongDanNopHoSo.nguoiTiepNhan.toLowerCase().replace(/(^|\s)\S/g, c => c.toUpperCase()) : "",
                    qrThongTinPhieu: xuLyHoSoContext.qrThongTinPhieu,
                }, "phieuHuongDan.phoi.docx", async (blob) => {
                    const res = await fileApi.UploadPdfBucketAsStream({
                        blob: blob,
                        fileName: `PhieuHuongDanBoSungHoSo.docx`,
                        folderName: `PhieuHuongDan${maTinh}`,
        ...
        (async () => {
            ...
            if (xuLyHoSoContext.qrThongTinPhieu && xuLyHoSoContext.xuatWord) {
                ...
                await generateDocxWithImages(huongDanNopHoSo?.urlPhoi || '', {
                    ...
                    saveAs(blob, 'Phiếu yêu cầu bổ sung, hoàn thiện hồ sơ.docx')
