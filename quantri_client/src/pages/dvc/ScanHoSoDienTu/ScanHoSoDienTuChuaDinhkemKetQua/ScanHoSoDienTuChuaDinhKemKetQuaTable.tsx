...
import { ScanHoSoDienTuChuaDinhKemKetQuaSearch } from "./ScanHoSoDienTuChuaDinhKemKetQuaSearch";
import { form } from "@formio/react";


const ScanHoSoDienTuChuaDinhKemKetQua = ({ daKySo }: { daKySo?: boolean }) => {
    const dispatch = useAppDispatch();
    const buttonActionContext = useButtonActionContext();
    const { data: user } = useAppSelector((state) => state.user);
    const { datas: hoSos, count, loading } = useAppSelector((state) => state.hoso);
    const { btnElememts } = useButtonActions({ maScreen: screenType["scan-ho-so-chua-scan-ket-qua"] });
    ...
    return (
        <LazyActions setSearchParams={setSearchParams}>
            <AntdSpace direction="vertical" style={{ width: "100%" }}>
                {btnElememts}
                <ScanHoSoDienTuChuaDinhKemKetQuaSearch daKySo={daKySo} setSearchParams={setSearchParams}></ScanHoSoDienTuChuaDinhKemKetQuaSearch>
                <AntdTable
                    loading={loading}
                    columns={columns}
                    dataSource={hoSos}
...
const HoSoTableWrapper = ({ daKySo }: { daKySo?: boolean }) => (
    <ButtonActionProvider>
        <ScanHoSoDienTuChuaDinhKemKetQua daKySo={daKySo} />
    </ButtonActionProvider>
);
