...
export const SearchHoSoToiHan = ({
  ...
  const onFinish = (values: ISearchHoSo) => {
    const formData: ISearchHoSo = {
      ...
      hoSoToiHan: form.getFieldValue("hoSoToiHan") ? form.getFieldValue("hoSoToiHan") : "get-all",
      pageNumber: 1
...
const TimKiemNangCao = ({ visible, setVisible, form, setSearchParams }: { setSearchParams: ComponentProps<typeof SearchHoSoToiHan>["setSearchParams"]; visible: boolean; setVisible: React.Dispatch<React.SetStateAction<boolean>>; form: FormInstance<ISearchHoSo> }) => {
  const onCancel = () => {
    setVisible(false);
  }
  const onOk = async () => {
