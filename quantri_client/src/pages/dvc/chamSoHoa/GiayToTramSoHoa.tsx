...
export const GiayToTramSoHoa = () => {
    ...
    useEffect(() => {
        if(tramSoHoaContext.soHoaData?.loaiSoHoa){
            form.setFieldValue("loaiSoHoa", tramSoHoaContext.soHoaData?.loaiSoHoa)
            setLoaiSoHoas((curr) => curr?.filter(x => x.value == tramSoHoaContext.soHoaData?.loaiSoHoa))
        }
    }, [tramSoHoaContext.soHoaData])
    const handleCancel = () => {
        tramSoHoaContext.setGiayToSoHoaModalVisible(false)
        tramSoHoaContext.setSoHoaData(undefined)
    }
    const onOk = async () => {
        const formData = await form.validateFields() as IGiayToSoHoa
        if(!tramSoHoaContext.soHoaData?.thanhPhanHoSoId){
            return;
        }
        const giayToSoHoa: AddGiayToSoHoaParams = {...formData, ...tramSoHoaContext.soHoaData, maHoSo: hoSo?.maHoSo, chuGiayTo: hoSo?.chuHoSo,}
        var res = await dispatch(AddGiayToSoHoa(giayToSoHoa)).unwrap()
        if(res.succeeded){
            toast.success("Số hóa thành công")
            tramSoHoaContext.soHoaData?.onOk()
        }
        handleCancel()
    ...
    return <AntdModal confirmLoading={loading} title="Thông tin số hóa" visible={true} onOk={onOk} handlerCancel={handleCancel} okText="Số hóa">
        <Form form={form} name="ThongTinGiayToSoHoa" layout="vertical" initialValues={{
            ...
            loaiSoHoa: tramSoHoaContext.soHoaData?.loaiSoHoa, ngayBanHanh:dayjs()}}>
            <Row gutter={8}>
            <Col span={8}>
