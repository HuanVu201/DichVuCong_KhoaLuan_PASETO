...
const noiDungBanGocThuLais = [
  { label: "GPLX ô tô", value: "GPLX ô tô" },
  { label: "GPLX xe máy", value: "GPLX xe máy" },
  { label: "GPLX ô tô & GPLX xe máy", value: "GPLX ô tô & GPLX xe máy" },
];

const TraKetQuaVaThuLaiBanGocModal = ({ setSearchHoSoParams }: { setSearchHoSoParams: React.Dispatch<React.SetStateAction<ISearchHoSo>> }) => {
  const buttonActionContext = useButtonActionContext();
  const [loading, setLoading] = useState<boolean>(false);
  const [data, setData] = useState<IHoSo>();
  const [form] = Form.useForm<IHoSo>();
  const dispatch = useAppDispatch();
  const signotecRef = useRef<SignotecRef>(null);
  ...
  const onOk = async (dghl: boolean = false) => {
    ...
    if (!form.getFieldValue("banGocThuLai")) {
      toast.error("Nhập thông tin bản gốc!");
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      const res = await hoSoApi.UpdateTraKetQuaHCC({
        ...
        banGocThuLai: form.getFieldValue("banGocThuLai"),
        soLuongBanGocThuLai: form.getFieldValue("soLuongBanGocThuLai"),
        dinhKemNhanKetQua: dinhKem,
        chuKyNguoiNhanKetQua: urlChuKyCongDan,
      });
      if (res.status == 200) {
        toast.success("Thao tác thành công!");
        buttonActionContext.setTraKetQuaVaThuLaiBanGocModalVisible(false);
  ...
  return (
    <AntdModal visible={true} title={`Trả kết quả và thu lại bản gốc: ${data?.maHoSo} (${data?.chuHoSo})`}
      width={700}
      handlerCancel={handlerCancel}
      footer={[
        <Button type="primary" onClick={async () => onOk(false)}>
          Trả kết quả
        </Button>,
      ...
      >
        ...
        >
          <Row gutter={[8, 8]}>
            ...
            <Col md={12} span={24}>
              <Form.Item label="Bản gốc thu lại" name="banGocThuLai" required>
                <AntdSelect
                  placeholder={"Chọn loại bản gốc"}
                  virtual={true}
                  generateOptions={{
                    model: noiDungBanGocThuLais,
                    label: "label",
                    value: "value",
                  }}
                />
...
export default TraKetQuaVaThuLaiBanGocModal;
