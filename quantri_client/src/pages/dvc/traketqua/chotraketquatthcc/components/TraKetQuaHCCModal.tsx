...
const TraKetQuaHCCModal = ({ setSearchHoSoParams }: { setSearchHoSoParams: React.Dispatch<React.SetStateAction<ISearchHoSo>> }) => {
  ...
  const onOk = async (dghl: boolean = false) => {
    ...
    try {
      ...
      if (res.status == 200) {
        ...
        buttonActionContext.setTraKetQuaHCCModalVisible(false);
  ...
  return (
    <AntdModal
      visible={true}
      title={`Trả kết quả HCC: ${data?.maHoSo} (${data?.chuHoSo})`}
      width={700}
...
export default TraKetQuaHCCModal;
