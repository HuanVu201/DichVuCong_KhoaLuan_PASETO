...
const PhieuBienNhanKetQuaModalVisible = () => {
    ...
    const handlerCancel = () => {
        ...
        buttonActionContext.setBienNhanKetQuaModalVisible(false)
    }
    ...
    const xuatLaiPhieuAction = () => {
        if (buttonActionContext.bienNhanKetQuaModalVisible) {
    ...
    const kySo = async () => {
        xuLyHoSoContext.handlerKySo("nguoiTraKetQua");
    }
    const xuatWord = async () => {
        xuLyHoSoContext.setXuatWord(true)
    }

    return <AntdModal okText="Tải phiếu" visible={true} title={"Phiếu biên nhận kết quả"} fullsizeScrollable handlerCancel={handlerCancel}
    footer={[
...
const PhieuBienNhanKetQuaModal = () => (
    <XuLyHoSoProvider>
        <PhieuBienNhanKetQuaModalVisible />
    </XuLyHoSoProvider>
);
