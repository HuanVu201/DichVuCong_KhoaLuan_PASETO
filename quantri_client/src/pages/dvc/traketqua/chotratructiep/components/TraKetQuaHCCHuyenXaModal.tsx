...
import { loaiNguoiNhanKetQuas } from "../../chotraketquatthcc/components/ChoTraKetQuaTTHCCTable";


const TraKetQuaHCCHuyenXaModal = ({ setSearchHoSoParams }: { setSearchHoSoParams: React.Dispatch<React.SetStateAction<ISearchHoSo>> }) => {
    ...
    setTimeout(async () => {
      try {
        ...
        const res = await hoSoApi.UpdateTraKetQuaHCC({
          ...
          dinhKemKetQua: data?.dinhKemKetQua,
          trichYeuKetQua: data?.trichYeuKetQua
        });
        if (res.status == 200) {
          toast.success("Thao tác thành công!");
          buttonActionContext.setTraKetQuaHCCHuyenXaModalVisible(false);
  ...
  return (
    <AntdModal
      visible={true}
      title={`Trả kết quả: ${data?.maHoSo} (${data?.chuHoSo})`}
      width={700}
      ...
      footer={[
        ...
        <Button type="primary" hidden={!data?.nguoiNhapDanhGiaText ? false : true} onClick={async () => {
          onOk(true)
        }}>
            ...
            <Col md={24} span={24}>
              <Form.Item label="Tệp đính kèm" name="urlDinhKem">
                <RegularUpload
                  ...
                  folderName={`TraKetQua_${data?.maHoSo}`}
                  dinhKem={dinhKem}
                />
              </Form.Item>
            </Col>
...
export default TraKetQuaHCCHuyenXaModal;
