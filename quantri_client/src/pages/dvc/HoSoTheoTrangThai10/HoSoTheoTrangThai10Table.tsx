...
const HoSoTheoTrangThai10Table = ({daKySo} : {daKySo ?: boolean}) => {
    const dispatch = useAppDispatch();
    const buttonActionContext = useButtonActionContext();
    const { data: user } = useAppSelector((state) => state.user);
    const { datas: hoSos, count, loading } = useAppSelector((state) => state.hoso);
    const { btnElememts } = useButtonActions({ maScreen: screenType["ky-so-ho-so-dien-tu"] });
    ...
    return (
        <LazyActions setSearchParams={setSearchParams}>
            <AntdSpace direction="vertical" style={{ width: "100%" }}>
                {btnElememts}
                <HoSoTheoTrangThai10Search daKySo = {daKySo} setSearchParams={setSearchParams}></HoSoTheoTrangThai10Search>
                <AntdTable
                    loading={loading}
                    columns={columns}
                    dataSource={hoSos}
...
const HoSoTableWrapper = ({daKySo} : {daKySo ?: boolean}) => (
    <ButtonActionProvider>
        <HoSoTheoTrangThai10Table daKySo = {daKySo} />
    </ButtonActionProvider>
);
