...
import { HoSoTheoTrangThai10Search } from "./HoSoTheoTrangThai10Search";
import { useHoSoDangXuLyColumn } from "@/features/hoso/hooks/useHoSoWithHanXuLy";


const HoSoTheoTrangThai10ChuaKySoTable = ({daKySo} : {daKySo ?: boolean}) => {
    const dispatch = useAppDispatch();
    const buttonActionContext = useButtonActionContext();
    const { data: user } = useAppSelector((state) => state.user);
    const { datas: hoSos, count, loading } = useAppSelector((state) => state.hoso);
    const { btnElememts } = useButtonActions({ maScreen: screenType["chua-ky-so-ho-so-dien-tu"] });
    ...
    const { columns } = useHoSoDangXuLyColumn(
    ...
    return (
        <LazyActions setSearchParams={setSearchParams}>
            <AntdSpace direction="vertical" style={{ width: "100%" }}>
                {btnElememts}
                <HoSoTheoTrangThai10Search daKySo = {false} setSearchParams={setSearchParams}></HoSoTheoTrangThai10Search>
                <AntdTable
                    loading={loading}
                    columns={columns}
                    dataSource={hoSos}
...
const HoSoTableWrapper = ({daKySo} : {daKySo ?: boolean}) => (
    <ButtonActionProvider>
        <HoSoTheoTrangThai10ChuaKySoTable daKySo = {daKySo} />
    </ButtonActionProvider>
);
