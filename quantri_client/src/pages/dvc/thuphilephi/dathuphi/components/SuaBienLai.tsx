...
const HINH_THUC_THANH_TOAN: SelectProps["options"] = [
  ...
  { value: "truc-tuyen", label: "Trực tuyến", disabled: true },
];
...
export const SuaBienLai = ({
  ...
  var hinhThucThanhToan = Form.useWatch("hinhThucThanhToan", form);
  const selectedTinh = Form.useWatch("tinhThanh", form);
  const selectedHuyen = Form.useWatch("quanHuyen", form);
  const [warningModalVisible, setWarningModalVisible] = useState<boolean>(true)
  ...
  const onOk: FormProps["onFinish"] = async () => {
    try {
      ...
      var postData = {
        ...
        data: {
          idYeuCauThanhToan: YeuCauThanhToanId.toString(),

          hinhThucThanhToan: form.getFieldValue("hinhThucThanhToan"),

          phi: loaiPhi == "phi" ? form.getFieldValue("phi") : undefined,
          lePhi: loaiPhi == "lephi" ? form.getFieldValue("lePhi") : undefined,
          tenPhiBienLai: loaiPhi == "phi" ? form.getFieldValue("tenPhiBienLai") : undefined,
          tenLePhiBienLai: loaiPhi == "lephi" ? form.getFieldValue("tenLePhiBienLai") : undefined,
      ...
      var resPay = await dispatch(UpdateBienLai(postData)).unwrap();
      reFetch();
      handleCancel();
    } catch (ex) {
  ...
  useEffect(() => {
    (async () => {
      ...
      form.setFieldsValue({
        ...
        nguoiNopTienBienLai: yeuCauThanhToan.data.chuHoSo,
        diaChiBienLai: yeuCauThanhToan.data.diaChiChuHoSo,
        ...
        tenPhiBienLai: yeuCauThanhToan.data.tenPhiBienLai ? yeuCauThanhToan.data.tenPhiBienLai :"Phí " + yeuCauThanhToan.data.tenTTHC ,
        tenLePhiBienLai: yeuCauThanhToan.data.tenLePhiBienLai ? yeuCauThanhToan.data.tenLePhiBienLai : "Lệ phí " + yeuCauThanhToan.data.tenTTHC,
        soGiayToNguoiNopTienBienLai: yeuCauThanhToan.data.soGiayToChuHoSo,
  ...
  return (
    <>
      ...
      >
                ...
                <Col md={12} span={24}>
                  <Form.Item name="hinhThucThanhToan" label="Hình thức thanh toán">
                    <Select
                      ...
                      disabled={hinhThucThanhToan == "truc-tuyen"}
                    />
                  </Form.Item>
                </Col>
          ...
          <AntdModal
          title="Sửa biên lai phí/ lệ phí sẽ hủy biên lai cũ và phát hành biên lai mới!"
          visible={warningModalVisible}
          handlerCancel={handleCancelWarning}
          width={500}
          style={{ fontSize: "22px", zIndex: 2 }}
          footer={
            <AntdSpace>
              <AntdButton onClick={handleCancelWarning} key={"1"}>
                Đóng
              </AntdButton>
            </AntdSpace>
          }/>
