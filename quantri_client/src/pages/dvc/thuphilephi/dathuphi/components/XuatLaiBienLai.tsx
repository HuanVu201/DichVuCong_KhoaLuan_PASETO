...
export const XuatLaiBienLai = ({
  ...
  const { data: yeuCauThanhToan,loading } = useAppSelector((state) => state.yeucauthanhtoan);
  const onOk: FormProps["onFinish"] = async () => {
    if(yeuCauThanhToan ) {
      if((loaiPhi == "phi" && parseInt(yeuCauThanhToan.phi) > 0) || (loaiPhi == "lephi" && parseInt(yeuCauThanhToan.lePhi) > 0))
     {
      var postData = {
        id: YeuCauThanhToanId[0].toString(),
        data: {
          idYeuCauThanhToan: YeuCauThanhToanId[0].toString(),
          loaiPhi: loaiPhi
        },
      };
      var resPay = await dispatch(InitBienLai(postData)).unwrap();
     } else toast.warning("Số tiền không hợp lệ")
    }
