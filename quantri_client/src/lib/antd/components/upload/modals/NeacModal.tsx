...
export const NeacModal = ({visible, setVisible, fileList, onSignedSucceed} : {onSignedSucceed: (newFile: string, oldFile: string) => void; visible: boolean; setVisible: React.Dispatch<React.SetStateAction<boolean>>; fileList: string | undefined}) => {
    ...
    const onOk = async () => {
        ...
        try {
            ...
            const signFiles: NEACSignFile[] = await Promise.all(fileList.split(ID_SEPARATE).map(async (filePath) => {
                const base64 = await uploadFileToBase64(filePath) as string
                const data = {
                    doc_id: uuid(),
            ...
            const res = await neacService.SignFile({
                ...
                transaction_id: currentSelectCertificateData.transaction_id ?? certificate?.data?.transaction_id,
                sign_files: signFiles
            })
            
            if(res.status == 200) {
                onSignedSucceed(res.data.data, fileList)
                toast.success("Ký số thành công")
                handlerCancel()
            } else {
                toast.success("Ký số thất bại")
            }
            setBtnLoading(false)
        } catch (error) {
            console.log(error);
    ...
    const onGetCertificate = async () => {
        ...
        try {
            setBtnLoading(true)
            const res = await neacService.GetCertificates({user_id: user_id, ca_name })
            if(res.status === 200) {
    ...
    return <AntdModal width={800} visible={visible} handlerCancel={handlerCancel} maskClosable={false} title={"Thông tin ký số"} 
        footer={
            <AntdSpace direction="horizontal">
                <AntdButton key={"1"} loading={btnLoading} onClick={onOk}>Đồng ý</AntdButton>
                ...
                <Col span={24}>
                    <Form.Item name={"user_id"} label="Tài khoản ký số">
                        <AntdSpace.Compact style={{ width: '100%' }}>
                            <Input />
                            <AntdButton loading={btnLoading} onClick={onGetCertificate}>Lấy chứng thư số</AntdButton>
                        </AntdSpace.Compact>
                    </Form.Item>
                </Col>
                <Col span={24}>
