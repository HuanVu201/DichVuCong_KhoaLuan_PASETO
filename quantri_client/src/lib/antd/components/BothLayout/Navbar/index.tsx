...
const NavbarLayout = ({ setSelectedMenuItem }: { setSelectedMenuItem: React.Dispatch<React.SetStateAction<IMenuPortal | null>> }) => {
  ...
  useEffect(() => {
    dispatch(SearchMenuPortal({ pageNumber: 1, pageSize: 1000 }));
  }, []);
  ...
  useEffect(() => {
    if (datas) {
      ...
      if (tmp) {
        const tmpWithHomePage = [{ key: "home", label: <Link to={primaryRoutes.portaldvc.home} key={"homepage"} ><HomeOutlined /></Link> } as any, ...tmp]
