...
import { Button, Collapse, Input, MenuProps, Row, Space, Table } from "antd";
...
import { DongBoThuTucModal } from "./modals/dongbothutuc/DongBoThuTucModal";
...
const ThuTucTable = () => {
  ...
  const items: MenuProps["items"] = [
    {
      label: (
        ...
        >
          <FileExcelOutlined style={{ color: "green" }} /> Thủ tục nhiều đơn vị sử dụng
        </button>
      ),
      key: "excel",
    },
  ...
  const exportThongKe = async (typeExport: string) => {
    // setLoading(true)
    const resExport = await exportThongKeApi.Exportthutucnhieudonvisudung(searchParams, typeExport)
    if (resExport.data) {
    ...
    <>
      <AntdSpace direction="vertical" style={{ width: "100%" }}>
        <Row>
          ...
          <Space size="small">
            <Button
              style={{ backgroundColor: '#16af27' }}
              type="primary"
              className="m-2"
              onClick={() => {
                thuTucContext.setDongBoThuTucModalVisible(true)
              }}
            >
              Đồng bộ thủ tục từ cổng DVCQG
            </Button>

          </Space>
        </Row>
        <ThuTucSearch setSearchParams={setSearchParams} items={items} />
        <div style={{ fontWeight: '500' }}>Tổng số <span style={{ color: 'red' }}>{count}</span> thủ tục</div>
        <AntdTable
          bordered
      ...
      {thuTucContext.dongBoThuTucModalVisible ? <DongBoThuTucModal /> : null}
