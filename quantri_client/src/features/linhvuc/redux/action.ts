...
export const SearchLinhVucTheoDonVi =
    createAsyncThunk<IPaginationResponse<ILinhVuc[]>, ISearchLinhVuc>("SearchLinhVucTheoDonVi", async (params, thunkApi) => {
        try {
            const res = await linhVucApi.SearchTheoDonVi(params)
            return res.data
        } catch (error) {
            return thunkApi.rejectWithValue(error)
        }
