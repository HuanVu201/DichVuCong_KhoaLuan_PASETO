import { useMemo } from 'react'
import { IDanhMucDiaBan } from '../models'
import { ColumnsType } from 'antd/es/table'
import { Popconfirm, Space } from 'antd'
import { CheckCircleOutlined, CloseCircleOutlined, DeleteOutlined, EditOutlined } from '@ant-design/icons'
import { useAppDispatch } from '../../../lib/redux/Hooks'
import { DeleteDanhMucDiaBan } from '../redux/action'
import { IBasePagination } from '../../../models'
import { useDiaBanContext } from '../contexts/DiaBanContext'

export const useColumn = (pagination: IBasePagination) => {
    const dispatch = useAppDispatch()
    const diaBanContext = useDiaBanContext()
    const columns = useMemo((): ColumnsType<IDanhMucDiaBan> => {
        return [
            {
                title: "STT",
                width: "5%",
                align: "center",
                render: (_, record, idx) => {
                    const pageNumber = pagination.pageNumber ?? 1
                    const pageSize = pagination.pageSize ?? 10
                    return <>{(pageNumber - 1) * pageSize + idx + 1}</>
                },
            },
            {
                title: "Tên địa bàn",
                key: "tenDiaBan",
                dataIndex: "tenDiaBan",
            },
            {
                title: "Mã địa bàn",
                key: "maDiaBan",
                dataIndex: "maDiaBan",
            },
            {
                title: "Thứ tự",
                key: "thuTu",
                dataIndex: "thuTu",
                align: 'center',
            },
            {
                title: "Sử dụng",
                key: "active",
                dataIndex: "active",
                align: 'center',
                render: (text) => text == true ? <CheckCircleOutlined style={{ color: 'green' }}></CheckCircleOutlined> : <CloseCircleOutlined style={{ color: 'red' }}></CloseCircleOutlined>
            },

            {
                title: "Thao tác",
                dataIndex: '',
                width: "10%",
                align: 'center',
                key: '',
                render: (_, record) => (
                    <Space direction="horizontal">
                        <EditOutlined style={{ color: "cornflowerblue" }} title="Xem chi tiết/Sửa" onClick={() => {
                            diaBanContext.setDiaBanId(record.id)
                            diaBanContext.setDiaBanModalVisible(true)
                        }} />
                        <Popconfirm
                            title='Xoá?'
                            onConfirm={() => {
                                dispatch(DeleteDanhMucDiaBan({ id: record.id, forceDelete: false }))
                            }}
                            okText='Xoá'
                            cancelText='Huỷ'
                        >
                            <DeleteOutlined style={{ color: "tomato" }} />
                        </Popconfirm>
                    </Space>
                )
            }
        ]
    }, [pagination])
    return { columns }
}