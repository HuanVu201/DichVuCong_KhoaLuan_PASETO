...
const YeuCauThanhToanChungThuc = ({setSearchHoSoParams}: {setSearchHoSoParams: React.Dispatch<React.SetStateAction<ISearchHoSo>>}) => {
    ...
    return <AntdModal confirmLoading={btnLoading} fullsizeScrollable  title={"YÊU CẦU THANH TOÁN CHỨNG THỰC"+ ` ${hoSo?.maHoSo ?? ""} ${hoSo?.chuHoSo ? `(${hoSo.chuHoSo})` : ""}`} visible={true} handlerCancel={handleCancel} onOk={onOk} okText="Gửi yêu cầu">
        <Form form={form}>  
            <Row gutter={8}>
                <Col span={24}>
