...
import {
  GetHoSo,
  XacNhanKetQua,
  YeuCauBCCILayKetQuaNhieuHoSoItemCode,
...
const YeuCauBCCILayKetQuaNhieuHoSoItemCodeModal = ({
  setSearchHoSoParams,
}: {
  ...
  const handleCancel = () => {
    ...
    buttonActionContext.setYeuCauBCCILayKetQuaNhieuHoSoModalVisible(false);
  };
  ...
  const onOk = async () => {
    if (buttonActionContext.selectedHoSos.length) {
      try {
        ...
        const res = await dispatch(YeuCauBCCILayKetQuaNhieuHoSoItemCode({ids:buttonActionContext.selectedHoSos})).unwrap();
        ...
        if(res.succeeded){
          ...
          if(count) toast.success(`Trả kết quả thành công ${count}/${buttonActionContext.selectedHoSos.length} hồ sơ`); else{
            toast.warning(`Trả kết quả thất bại ${buttonActionContext.selectedHoSos.length-count}/${buttonActionContext.selectedHoSos.length} hồ sơ`)
          }
        }else {
...
export default YeuCauBCCILayKetQuaNhieuHoSoItemCodeModal;
