...
const LienThongHeThongLLTP = ({setSearchHoSoParams} : {setSearchHoSoParams: React.Dispatch<React.SetStateAction<ISearchHoSo>>}) => {
    ...
    const onOk = async () => {
        ...
        try {
            ...
            // try{
            //     const jsonRes : {status: string; description: string; id?: string} = JSON.parse(res.data)
            //     if(jsonRes.status == "501" && jsonRes.description == "Lưu thành công"){
            //         setSearchHoSoParams((curr) => ({...curr}))
            //         toast.success("Gửi hồ sơ thành công");
            //         handlerCancel()
            //     } else {
            //         toast.warn(jsonRes.description || "Có lỗi xảy ra, vui lòng liên hệ quản trị hệ thống");
            //     }
            // } catch(e){
            //     console.log(e);
            //     setBtnLoading(false)
