...
export const SearchBaoCao01 =
    ...
    export const ExportExcelBaoCao01 =
    createAsyncThunk<Stream, ISearchPhieuKhaoSat>("ExportExcelBaoCao01", async (params, thunkApi) => {
        try {
            const res = await PhieuKhaoSatApi.ExportExcelBaoCao01(params)
            return res.data
        } catch (error) {
            return thunkApi.rejectWithValue(error)
        }
