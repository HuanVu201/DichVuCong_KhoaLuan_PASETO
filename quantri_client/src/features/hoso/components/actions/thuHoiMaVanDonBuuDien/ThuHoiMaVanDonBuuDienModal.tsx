...
import { ThuHoiMaVanDonBuuDienRequest } from "@/features/hoso/services";
...
const ThuHoiMaVanDonBuuDienModal = ({
  ...
  const handleCancel = () => {
    buttonActionContext.setThuHoiMaVanDonBuuDienModalVisible(false);
    form.resetFields();
  ...
  const onOk = async () => {
    if (buttonActionContext.selectedHoSos.length) {
      try {
        setBtnLoading(true)
        var req = {
          hoSoIds: buttonActionContext.selectedHoSos
        } 
        const res = await dispatch(ThuHoiMaVanDonBuuDien(req)).unwrap();
...
export default ThuHoiMaVanDonBuuDienModal;
