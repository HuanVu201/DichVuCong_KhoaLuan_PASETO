...
function DetailThuTucModal({ thuTucId, detailThuTucModalVisible, setDetailThuTucModalVisible }: {
    thuTucId: string | undefined,
    detailThuTucModalVisible: boolean,
    setDetailThuTucModalVisible: React.Dispatch<React.SetStateAction<boolean>
    >
}) {
    ...
    useEffect(() => {
        (async () => {
            if (thuTucId && detailThuTucModalVisible) {
                const resThuTuc = await thuTucPortalService.Get(thuTucId);

                if (resThuTuc?.data?.data) {
                    setThuTuc(resThuTuc.data.data);
                } else {
                    toast.error("Lỗi lấy thông tin chi tiết thủ tục!")
                }
            }
        })();
    }, [thuTucId, detailThuTucModalVisible]);
    ...
    const handlerCancel = () => {
        setDetailThuTucModalVisible(false)
    }

    return (
        <AntdModal visible={detailThuTucModalVisible} title={`Thông tin chi tiết thủ tục`} fullsizeScrollable handlerCancel={handlerCancel}
            footer={[<Button key="back" onClick={handlerCancel} >
                Đóng
            </Button>
            ]}
        >
...
export default DetailThuTucModal;
