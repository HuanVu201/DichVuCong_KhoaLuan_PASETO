...
export const DangKyNhanKetQua = ({ form }: { form: FormInstance<IHoSo> }) => {
    ...
    useEffect(() => {
        if (hinhThucTra != "1") {
            ...
            const maQuanHuyen =  window.objDataCSDLDanCu?.ThuongTru.MaQuanHuyen
            const maPhuongXa =  window.objDataCSDLDanCu?.ThuongTru.MaPhuongXa
            if (uyQuyenHoSo) {
                const tenTinh = maTinh?.find(x => x.maDiaBan == maTinhThanh)
                const tenQuan = maHuyen?.find(x => x.maDiaBan == maTinhThanh + "." +maQuanHuyen)
                const tenXa = maXa?.find(x => x.maDiaBan == maTinhThanh + "."+maQuanHuyen+"."+maPhuongXa)
                ...
                form.setFieldValue("bcci_quanHuyen", maTinhThanh + "." +maQuanHuyen)
                form.setFieldValue("bcci_xaPhuong", maTinhThanh + "."+maQuanHuyen+"."+maPhuongXa)
    ...
    }, [hinhThucTra, uyQuyenHoSo, maTinh, maHuyen, maXa])
