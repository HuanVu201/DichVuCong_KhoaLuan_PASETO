...
export const DangKyNhanKetQua = ({ form }: { form: FormInstance<IHoSo> }) => {
    ...
    useEffect(() => {
        let parsedBCCI;
        try {
            parsedBCCI = hoSo?.dangKyNhanHoSoQuaBCCIData ? JSON.parse(hoSo.dangKyNhanHoSoQuaBCCIData) : undefined
            form.setFieldValue("bcci_tinhThanh", parsedBCCI.tinhThanh)
            form.setFieldValue("bcci_quanHuyen", parsedBCCI.quanHuyen)
            form.setFieldValue("bcci_xaPhuong", parsedBCCI.xaPhuong)
            form.setFieldValue("bcci_diaChi", parsedBCCI.diaChi)
        } catch (error) {
        } 
    }, [hoSo])
    ...
    useEffect(() => {
        if (hinhThucTra != "1") {
            ...
            const maTinhThanh = form.getFieldValue("bcci_tinhThanh") 
            const maQuanHuyen = form.getFieldValue("bcci_quanHuyen") 
            const maPhuongXa = form.getFieldValue("bcci_xaPhuong")
            if (uyQuyenHoSo) {
                const tenTinh = maTinh?.find(x => x.maDiaBan == maTinhThanh)
                const tenQuan = maHuyen?.find(x => x.maDiaBan == maQuanHuyen)
                const tenXa = maXa?.find(x => x.maDiaBan ==maPhuongXa)
                ...
                form.setFieldValue("bcci_quanHuyen", maQuanHuyen)
                form.setFieldValue("bcci_xaPhuong",maPhuongXa)
    ...
    }, [hinhThucTra, uyQuyenHoSo, maTinh, maHuyen, maXa, hoSo])
    ...
    return <>
        {hinhThucTra === "1" ? <Row gutter={[8, 0]}>
            <Form.Item><Input hidden name="bcci_tenQuanHuyen"></Input></Form.Item>
            <Form.Item><Input hidden name="bcci_tenXaPhuong"></Input></Form.Item>
            <Form.Item><Input hidden name="bcci_tenTinhThanh"></Input></Form.Item>
            <Col span={8}>
