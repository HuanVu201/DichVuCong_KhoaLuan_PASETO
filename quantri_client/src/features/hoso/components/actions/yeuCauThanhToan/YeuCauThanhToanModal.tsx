...
const YeuCauThanhToanModal = ({ setSearchHoSoParams }: { setSearchHoSoParams: React.Dispatch<React.SetStateAction<ISearchHoSo>> }) => {
    ...
    const [xacNhanYcttModalVisible, setXacNhanYcttModalVisible] = useState<boolean>(false);
    useEffect(() => {
    ...
    const onOk = async () => {
        if (hoSo?.yeuCauThanhToans && hoSo?.yeuCauThanhToans.length > 0) {
            setXacNhanYcttModalVisible(true)
        } else {
            onSubmit();
        }
    }
    const onSubmit = async () => {
        ...
        setXacNhanYcttModalVisible(false)
        if (!btnLoading) {
            if (buttonActionContext.selectedHoSos.length) {
                try {
                    setBtnLoading(true)
                    const res = await dispatch(YeuCauThuPhi({
                        id: buttonActionContext.selectedHoSos[0] as string, data: {
    ...
    return <AntdModal confirmLoading={btnLoading} title={"YÊU CẦU THU PHÍ, LỆ PHÍ HỒ SƠ" + ` ${hoSo?.maHoSo ?? ""} ${hoSo?.chuHoSo ? `(${hoSo.chuHoSo})` : ""}`} visible={true} handlerCancel={handleCancel}
        ...
        {xacNhanYcttModalVisible ? <AntdModal confirmLoading={btnLoading} title={"YÊU CẦU THU PHÍ, LỆ PHÍ HỒ SƠ" + ` ${hoSo?.maHoSo ?? ""} ${hoSo?.chuHoSo ? `(${hoSo.chuHoSo})` : ""}`} visible={true} handlerCancel={handleCancel}
            onOk={onSubmit} okText="Xác nhận" >
            Hồ sơ đã có yêu cầu thu phí. Bạn có muốn tiếp tục?
        </AntdModal> : null}
    </AntdModal>
}
