...
import { Checkbox, CheckboxProps, Col, Row, SelectProps, Typography } from "antd"
import { AntdDivider, AntdSelect, AntdSpace } from "@/lib/antd/components"
...
export const DanhSachQuyTrinh = ({ form }: { form: FormInstance<ChuyenNoiBoFormParam> }) => {
    ...
    const onSelectNguoiDung: SelectProps["onChange"] = (e) => {
    ...
    const nguoiDungOpTions: SelectProps["options"] = nguoiDungNhomNguoiDungs
        ?.filter(x => x.id !== user?.id)
        .map((nguoiDung, idx2) => ({
            label: nguoiDung.fullName,
            value: nguoiDung.id
        }));
    ...
    const onDeselect = (e: any) => {
        setSelectedNguoiDungs(
            selectedNguoiDungs?.filter((x) => x != e)
        );
    }
