...
import { LOAIDULIEUKETNOI } from "../../models";
...
export const validateHoSoLLTPVNeID = (loaiDuLieuKetNoi: keyof typeof LOAIDULIEUKETNOI) => {
    if(loaiDuLieuKetNoi == "LLTPVneid" || loaiDuLieuKetNoi == "LLTPVneidUyQuyen"){
        toast.info("Hồ sơ thuộc VNeID vui lòng không thực hiện chức năng này")
        return true;
    }
    return false;
}

export const validateHoSoDVCLT = (loaiDuLieuKetNoi: keyof typeof LOAIDULIEUKETNOI) => {
    console.log(loaiDuLieuKetNoi);
    
    if(loaiDuLieuKetNoi == "LTKT" || loaiDuLieuKetNoi == "LTKS"){
        toast.info("Hồ sơ thuộc DVCLT, vui lòng không sử dụng chức năng này")
        return true;
    }
    return false;
}