...
export const PhiLePhi = ({
  ...
  hasCharge,
  defaultSelected,
  overrideUpdateSoTien,
  updateSoTienDeps,
}: {
  updateSoTienDeps?: React.DependencyList;
  overrideUpdateSoTien?: (data: IPhiLePhi[]) => void;
  defaultSelected?: string;
  hasCharge: string[];
  form: FormInstance<any>;
  hinhThucThuOptions: SelectProps["options"];
  phiLePhis: IPhiLePhi[] | undefined;
}) => {
  // const { duLieuThemHoSo } = useAppSelector(state => state.truonghopthutuc)
  const [searchParams, setSearchParams] = useState<ISearchPhiLePhi>({
    pageNumber: 1,
    pageSize: 100,
    reFetch: true,
  });
  ...
  const tongTien = Form.useWatch("tongTien", form);
  ...
  useEffect(() => {
    if (phiLePhis && phiLePhis.length) {
      ...
      setSelectedRowKeys(phiLePhis.flatMap((x) => x.id));
  ...
  useEffect(() => {
    ...
    overrideUpdateSoTien
      ? overrideUpdateSoTien(selectedRows)
  ...
  const moneyToText = numberToCurrencyText(tongTien);
    ...
    <>
      <Row gutter={[8, 0]}>
        ...
        {hasCharge.includes(hinhThucThu) ? (
          <>
            <Col md={6} span={24}>
              <Form.Item name="phiThu" label="Phí thu (VNĐ)">
                <InputNumber
                  style={{ width: "100%" }}
                  disabled
                  defaultValue={0}
            ...
            <Col md={6} span={24}>
              <Form.Item name="lePhiThu" label="Lệ phí thu (VNĐ)">
                <InputNumber
                  style={{ width: "100%" }}
                  disabled
                  defaultValue={0}
            ...
            <Col md={6} span={24}>
              <Form.Item
                name="tongTien"
                label={`Tổng tiền (VNĐ): ${moneyToText}`}
                rules={
                  hasCharge.includes(hinhThucThu)
                    ? [
                        {
