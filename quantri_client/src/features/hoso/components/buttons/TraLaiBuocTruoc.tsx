...
export interface TraLaiBuocTruocProps  extends BaseActionProps{
    actionName : string
}
// sử dụng comp này bọc ngoài do không thể sử dụng customhook, và các hook bth ở trong useMemo, useEffect
export const TraLaiBuocTruocWrapper = (props: TraLaiBuocTruocProps) => {
    
    return <TraLaiBuocTruoc {...props}></TraLaiBuocTruoc>
}

const TraLaiBuocTruoc = ({actionName, colorCode, iconName}: TraLaiBuocTruocProps) => {
    const buttonActionContext = useButtonActionContext()
    const onClick = () => {
        if(buttonActionContext.selectedHoSos.length){
            buttonActionContext.setTraLaiBuocTruocModalVisible(true)
        }else {
