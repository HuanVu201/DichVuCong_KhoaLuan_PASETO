...
export interface FakeTuChoiBTPHoSoProps extends BaseActionProps{
    actionName : string
}
// sử dụng comp này bọc ngoài do không thể sử dụng customhook, và các hook bth ở trong useMemo, useEffect
export const FakeTuChoiBTPHoSoWrapper = (props: FakeTuChoiBTPHoSoProps) => {
    
    return <FakeTuChoiBTPHoSo {...props}></FakeTuChoiBTPHoSo>
}

const FakeTuChoiBTPHoSo = ({actionName, colorCode,iconName}: FakeTuChoiBTPHoSoProps) => {
    const buttonActionContext = useButtonActionContext()
    const onClick = () => {
        if(buttonActionContext.selectedHoSos.length){
            console.log("???");
            
            buttonActionContext.setFakeTuChoiBTPHoSoModalVisible(true)
        }else {
