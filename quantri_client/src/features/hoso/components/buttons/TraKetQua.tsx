...
export const MaDonViTTHCC = "0A97F0DB-325A-6205-69D4-F7131F368F64"
export interface TraKetQuaProps extends BaseActionProps {
    actionName : string
}
// sử dụng comp này bọc ngoài do không thể sử dụng customhook, và các hook bth ở trong useMemo, useEffect
export const TraKetQuaWrapper = (props: TraKetQuaProps) => {
    
    return <TraKetQua {...props}></TraKetQua>
}

const TraKetQua = ({actionName, colorCode, iconName}: TraKetQuaProps) => {
    const buttonActionContext = useButtonActionContext()
    const onClick = () => {
        if(buttonActionContext.selectedHoSos.length){
            buttonActionContext.setDanhGiaHoSoModalVisible(true)
        }else {
