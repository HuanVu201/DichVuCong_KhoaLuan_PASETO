...
export interface TuChoiDuThaoProps extends BaseActionProps{
    actionName : string
}
// sử dụng comp này bọc ngoài do không thể sử dụng customhook, và các hook bth ở trong useMemo, useEffect
export const TuChoiDuThaoWrapper = (props: TuChoiDuThaoProps) => {
    
    return <TuChoiDuThao {...props}></TuChoiDuThao>
}

const TuChoiDuThao = ({actionName, iconName, colorCode}: TuChoiDuThaoProps) => {
    const buttonActionContext = useButtonActionContext()
    const onClick = () => {
        if(buttonActionContext.selectedHoSos.length){
            buttonActionContext.setTuChoiDuThaoModalVisible(true)
        }else {
