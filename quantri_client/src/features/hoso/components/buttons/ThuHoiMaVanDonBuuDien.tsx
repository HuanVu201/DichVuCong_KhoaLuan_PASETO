...
export interface ThuHoiMaVanDonBuuDienModalProps extends BaseActionProps {
  actionName: string;
}
// sử dụng comp này bọc ngoài do không thể sử dụng customhook, và các hook bth ở trong useMemo, useEffect
export const ThuHoiMaVanDonBuuDienModalWrapper = (
  props: ThuHoiMaVanDonBuuDienModalProps
) => {
  return <ThuHoiMaVanDonBuuDienModal {...props}></ThuHoiMaVanDonBuuDienModal>;
};

const ThuHoiMaVanDonBuuDienModal = ({
  actionName,
  iconName,
  colorCode,
}: ThuHoiMaVanDonBuuDienModalProps) => {
  const buttonActionContext = useButtonActionContext();
  const onClick = () => {
    if ((buttonActionContext.selectedHoSos.length >= 1)) {
      buttonActionContext.setThuHoiMaVanDonBuuDienModalVisible(true);
    } else {
