...
export interface DongDauProps extends BaseActionProps{
    actionName : string
}
// sử dụng comp này bọc ngoài do không thể sử dụng customhook, và các hook bth ở trong useMemo, useEffect
export const DongDauWrapper = (props: DongDauProps) => {
    
    return <DongDau {...props}></DongDau>
}

const DongDau = ({actionName, colorCode, iconName}: DongDauProps) => {
    const buttonActionContext = useButtonActionContext()
    const onClick = () => {
        if(buttonActionContext.selectedHoSos.length){
            buttonActionContext.setDongDauModalVisible(true)
        }else {
