...
export interface DuThaoXinLoiHoSoProps extends BaseActionProps{
    actionName : string
}
// sử dụng comp này bọc ngoài do không thể sử dụng customhook, và các hook bth ở trong useMemo, useEffect
export const DuThaoXinLoiHoSoWrapper = (props: DuThaoXinLoiHoSoProps) => {
    
    return <DuThaoXinLoiHoSo {...props}></DuThaoXinLoiHoSo>
}

const DuThaoXinLoiHoSo = ({actionName, iconName, colorCode}: DuThaoXinLoiHoSoProps) => {
    const buttonActionContext = useButtonActionContext()
    const onClick = () => {
        if(buttonActionContext.selectedHoSos.length){
            buttonActionContext.setDuThaoXinLoiHoSoModalVisible(true)
        }else {
