...
export interface YeuCauThanhToanVaXacNhanKqProps extends BaseActionProps {
  actionName: string;
}
// sử dụng comp này bọc ngoài do không thể sử dụng customhook, và các hook bth ở trong useMemo, useEffect
export const YeuCauThanhToanVaXacNhanKqWrapper = (
  props: YeuCauThanhToanVaXacNhanKqProps
) => {
  return <YeuCauThanhToanVaXacNhanKq {...props}></YeuCauThanhToanVaXacNhanKq>;
};

const YeuCauThanhToanVaXacNhanKq = ({
  actionName,
  colorCode,
  iconName,
}: YeuCauThanhToanVaXacNhanKqProps) => {
  const buttonActionContext = useButtonActionContext();
  const onClick = () => {
    if (buttonActionContext.selectedHoSos.length) {
      buttonActionContext.setYeuCauThanhToanVaXacNhanKqModalVisible(true);
    } else {
