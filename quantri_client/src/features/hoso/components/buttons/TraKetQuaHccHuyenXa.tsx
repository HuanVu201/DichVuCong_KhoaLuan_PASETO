...
export interface TraKetQuaHCCHuyenXaProps extends BaseActionProps {
  actionName: string
}
// sử dụng comp này bọc ngoài do không thể sử dụng customhook, và các hook bth ở trong useMemo, useEffect
export const TraKetQuaHCCHuyenXaWrapper = (props: TraKetQuaHCCHuyenXaProps) => {

  return <TraKetQuaHCCHuyenXa {...props}></TraKetQuaHCCHuyenXa>
}

const TraKetQuaHCCHuyenXa = ({ actionName, colorCode, iconName }: TraKetQuaHCCHuyenXaProps) => {
  ...
  const onClick = async () => {
    ...
    if (buttonActionContext.selectedHoSos.length) {
      if (checkThuPhi && checkDinhKemKetQua && checkKySo) {
        buttonActionContext.setTraKetQuaHCCHuyenXaModalVisible(true)
      }
    } else {
