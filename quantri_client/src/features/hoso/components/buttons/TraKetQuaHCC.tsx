...
export interface TraKetQuaHCCProps extends BaseActionProps {
  actionName: string
}
// sử dụng comp này bọc ngoài do không thể sử dụng customhook, và các hook bth ở trong useMemo, useEffect
export const TraKetQuaHCCWrapper = (props: TraKetQuaHCCProps) => {

  return <TraKetQuaHCC {...props}></TraKetQuaHCC>
}

const TraKetQuaHCC = ({ actionName, colorCode, iconName }: TraKetQuaHCCProps) => {
  ...
  const onClick = async () => {
    ...
    if (buttonActionContext.selectedHoSos.length) {
      if (checkThuPhi && checkDinhKemKetQua && checkKySo) {
        buttonActionContext.setTraKetQuaHCCModalVisible(true)
      }
    } else {
