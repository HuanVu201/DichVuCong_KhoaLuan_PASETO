...
export interface BoSungHoSoGuiVBDLISModalProps extends BaseActionProps {
  actionName: string;
}
// sử dụng comp này bọc ngoài do không thể sử dụng customhook, và các hook bth ở trong useMemo, useEffect
export const BoSungHoSoGuiVBDLISModalWrapper = (
  props: BoSungHoSoGuiVBDLISModalProps
) => {
  return <BoSungHoSoGuiVBDLISModal {...props}></BoSungHoSoGuiVBDLISModal>;
};

const BoSungHoSoGuiVBDLISModal = ({
  actionName,
  iconName,
  colorCode,
}: BoSungHoSoGuiVBDLISModalProps) => {
  const buttonActionContext = useButtonActionContext();
  const onClick = () => {
    if ((buttonActionContext.selectedHoSos.length >= 1)) {
      buttonActionContext.setBoSungHoSoGuiVBDLISModalVisible(true);
    } else {
