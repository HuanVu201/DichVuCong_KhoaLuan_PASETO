...
export interface ChuyenBuocNhanhHoSoProps extends BaseActionProps{
    actionName : string
}
// sử dụng comp này bọc ngoài do không thể sử dụng customhook, và các hook bth ở trong useMemo, useEffect
export const ChuyenBuocNhanhHoSoWrapper = (props: ChuyenBuocNhanhHoSoProps) => {
    
    return <ChuyenBuocNhanhHoSo {...props}></ChuyenBuocNhanhHoSo>
}

const ChuyenBuocNhanhHoSo = ({actionName, colorCode, iconName}: ChuyenBuocNhanhHoSoProps) => {
    const buttonActionContext = useButtonActionContext()
    const onClick = () => {
        if(buttonActionContext.selectedHoSos.length){
            buttonActionContext.setChuyenBuocNhanhHoSoModalVisible(true)
        }else {
