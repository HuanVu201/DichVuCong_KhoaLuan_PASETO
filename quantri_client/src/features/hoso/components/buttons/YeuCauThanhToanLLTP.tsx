...
export interface YeuCauThanhToanLLTPProps extends BaseActionProps {
    actionName: string
}
// sử dụng comp này bọc ngoài do không thể sử dụng customhook, và các hook bth ở trong useMemo, useEffect
export const YeuCauThanhToanLLTPWrapper = (props: YeuCauThanhToanLLTPProps) => {

    return <YeuCauThanhToanLLTP {...props}></YeuCauThanhToanLLTP>
}


const YeuCauThanhToanLLTP = ({ actionName, colorCode, iconName }: YeuCauThanhToanLLTPProps) => {
    ...
    const onClick = () => {
        ...
        if (buttonActionContext.selectedHoSos.length && buttonActionContext.selectedHoSo?.trangThaiPhiLePhi == true) {
            buttonActionContext.setYeuCauThanhToanLLTPModalVisible(true)
        }
        else if (buttonActionContext.selectedHoSo?.trangThaiPhiLePhi == false) {
            toast.info("Thủ tục hành chính không có phí, lệ phí")
        }
        else {
            toast.info("Vui lòng chọn hồ sơ trước")
        }
