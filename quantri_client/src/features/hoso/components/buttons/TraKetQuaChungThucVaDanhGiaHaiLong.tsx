...
export interface TraKetQuaChungThucVaDanhGiaHaiLongProps extends BaseActionProps {
    actionName : string
}
// sử dụng comp này bọc ngoài do không thể sử dụng customhook, và các hook bth ở trong useMemo, useEffect
export const TraKetQuaChungThucVaDanhGiaHaiLongWrapper = (props: TraKetQuaChungThucVaDanhGiaHaiLongProps) => {
    
    return <TraKetQuaChungThucVaDanhGiaHaiLong {...props}></TraKetQuaChungThucVaDanhGiaHaiLong>
}

const TraKetQuaChungThucVaDanhGiaHaiLong = ({actionName, colorCode, iconName}: TraKetQuaChungThucVaDanhGiaHaiLongProps) => {
    const buttonActionContext = useButtonActionContext()
    const onClick = () => {
        if(buttonActionContext.selectedHoSos.length){
            buttonActionContext.setTraKetQuaChungThucVaDanhGiaHaiLongModalVisible(true)
        }else {
