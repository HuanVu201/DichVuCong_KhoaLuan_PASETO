...
export interface ThuHoiHoSoProps extends BaseActionProps{
    actionName : string
}
// sử dụng comp này bọc ngoài do không thể sử dụng customhook, và các hook bth ở trong useMemo, useEffect
export const ThuHoiHoSoWrapper = (props: ThuHoiHoSoProps) => {
    
    return <ThuHoiHoSo {...props}></ThuHoiHoSo>
}

const ThuHoiHoSo = ({actionName, colorCode, iconName}: ThuHoiHoSoProps) => {
    const buttonActionContext = useButtonActionContext()
    const onClick = () => {
        if(buttonActionContext.selectedHoSos.length){
            buttonActionContext.setThuHoiHoSoModalVisible(true)
        }else {
