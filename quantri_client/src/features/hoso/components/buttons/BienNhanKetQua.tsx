...
export interface BienNhanKetQuaProps extends BaseActionProps {
  actionName: string;
}
// sử dụng comp này bọc ngoài do không thể sử dụng customhook, và các hook bth ở trong useMemo, useEffect
export const BienNhanKetQuaWrapper = (props: BienNhanKetQuaProps) => {
  return <BienNhanKetQua {...props}></BienNhanKetQua>;
};

const BienNhanKetQua = ({
  actionName,
  colorCode,
  iconName,
}: BienNhanKetQuaProps) => {
  ...
  const onClick = () => {
    if (buttonActionContext.selectedHoSos.length) {
      buttonActionContext.setBienNhanKetQuaModalVisible(true);
    } else {
