...
export interface CapNhatBoSungHoSoProps extends BaseActionProps{
    actionName : string
}
// sử dụng comp này bọc ngoài do không thể sử dụng customhook, và các hook bth ở trong useMemo, useEffect
export const CapNhatBoSungHoSoWrapper = (props: CapNhatBoSungHoSoProps) => {
    
    return <CapNhatBoSungHoSo {...props}></CapNhatBoSungHoSo>
}

const CapNhatBoSungHoSo = ({actionName, colorCode, iconName}: CapNhatBoSungHoSoProps) => {
    const buttonActionContext = useButtonActionContext()
    const onClick = () => {
        if(buttonActionContext.selectedHoSos.length){
            buttonActionContext.setCapNhatBoSungHoSoModalVisible(true)
        }else {
