...
const DuThaoTraLaiXinRutHoSo = ({actionName, iconName, colorCode}: DuThaoTraLaiXinRutHoSoProps) => {
    ...
    return <AntdButton onClick={onClick}  style={{backgroundColor: colorCode, color: "#333"}} icon={iconName ? ICON_HOLDER[iconName] : undefined} disabled={buttonActionContext.selectedHoSos.length > 1}>{actionName}</AntdButton>
}