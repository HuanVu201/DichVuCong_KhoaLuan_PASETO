...
export interface KetThucXuLyNhieuHoSoProps extends BaseActionProps{
    actionName : string
}
// sử dụng comp này bọc ngoài do không thể sử dụng customhook, và các hook bth ở trong useMemo, useEffect
export const KetThucXuLyNhieuHoSoWrapper = (props: KetThucXuLyNhieuHoSoProps) => {
    
    return <KetThucXuLyNhieuHoSo {...props}></KetThucXuLyNhieuHoSo>
}

const KetThucXuLyNhieuHoSo = ({actionName, colorCode, iconName}: KetThucXuLyNhieuHoSoProps) => {
    const buttonActionContext = useButtonActionContext()
    const onClick = () => {
        if(buttonActionContext.selectedHoSos.length){
            buttonActionContext.setKetThucXuLyNhieuHoSoModalVisible(true)
        }else {
