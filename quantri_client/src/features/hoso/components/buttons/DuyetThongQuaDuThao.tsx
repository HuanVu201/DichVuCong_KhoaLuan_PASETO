...
export interface DuyetThongQuaDuThaoProps extends BaseActionProps{
    actionName : string
}
// sử dụng comp này bọc ngoài do không thể sử dụng customhook, và các hook bth ở trong useMemo, useEffect
export const DuyetThongQuaDuThaoWrapper = (props: DuyetThongQuaDuThaoProps) => {
    
    return <DuyetThongQuaDuThao {...props}></DuyetThongQuaDuThao>
}

const DuyetThongQuaDuThao = ({actionName, iconName, colorCode}: DuyetThongQuaDuThaoProps) => {
    const buttonActionContext = useButtonActionContext()
    const onClick = () => {
        if(buttonActionContext.selectedHoSos.length){
            buttonActionContext.setDuyetThongQuaDuThaoModalVisible(true)
        }else {
