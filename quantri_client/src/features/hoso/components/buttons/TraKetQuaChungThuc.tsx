...
export interface TraKetQuaChungThucProps extends BaseActionProps {
    actionName : string
}
// sử dụng comp này bọc ngoài do không thể sử dụng customhook, và các hook bth ở trong useMemo, useEffect
export const TraKetQuaChungThucWrapper = (props: TraKetQuaChungThucProps) => {
    
    return <TraKetQuaChungThuc {...props}></TraKetQuaChungThuc>
}

const TraKetQuaChungThuc = ({actionName, colorCode, iconName}: TraKetQuaChungThucProps) => {
    const buttonActionContext = useButtonActionContext()
    const onClick = () => {
        if(buttonActionContext.selectedHoSos.length){
            buttonActionContext.setTraKetQuaChungThucModalVisible(true)
        }else {
