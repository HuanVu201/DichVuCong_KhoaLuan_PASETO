...
export interface TiepNhanGuiVBDLISModalProps extends BaseActionProps {
  actionName: string;
}
// sử dụng comp này bọc ngoài do không thể sử dụng customhook, và các hook bth ở trong useMemo, useEffect
export const TiepNhanGuiVBDLISModalWrapper = (
  props: TiepNhanGuiVBDLISModalProps
) => {
  return <TiepNhanGuiVBDLISModal {...props}></TiepNhanGuiVBDLISModal>;
};

const TiepNhanGuiVBDLISModal = ({
  actionName,
  iconName,
  colorCode,
}: TiepNhanGuiVBDLISModalProps) => {
  const buttonActionContext = useButtonActionContext();
  const onClick = () => {
    if ((buttonActionContext.selectedHoSos.length >= 1)) {
      buttonActionContext.setTiepNhanGuiVBDLISModalVisible(true);
    } else {
