...
export const useHoSoDangXuLyLienThong = (
  ...
  > => {
    return [
      ...
      },
      {
        ...
        render: (_, record) => {
          return <>
            <div>Đơn vị: {record.tenDonVi}</div>
            {record.tenNguoiXuLyTruoc}
          </>
        }
      },
      {
      ...
      },
      {
        ...
        render: (_, record) => {
          ...
          return (
            <div>
              {hanXuLyCaNhan ? hanXuLyCaNhan.format(FORMAT_DATE_WITHOUT_TIME) : ""}
            </div>
          );
        }
      },
      ...
      {
        title: <p style={{ fontSize: 16, textAlign: 'center' }}>Văn bản liên thông</p>,
        key: "ketQua",
        render: (_, record) => {
          return (
            <>
              ...
              <div style={{ fontWeight: '500' }}>
                - Số ký hiệu: {record.soKyHieuHSLT}
              </div>
              {record.trichyeuHSLT ?
                <div style={{ display: 'flex', flexDirection: 'row' }}>
                  {/* <span>- </span> */}
              ...
              <div style={{ display: 'flex', flexDirection: 'row', fontWeight: '500', alignItems: 'center' }}>
                <span>- Đính kèm:</span>
                {record.dinhKemHSLT?.split(ID_SEPARATE).map((dinhKem, idx) =>
                  <AntdSpace direction="horizontal" onClick={() => callApiAndDisplayFile(dinhKem)} key={idx}>
                    {''} <LinkOutlined style={{ color: "yellowgreen" }} role='button' title={dinhKem.substring(dinhKem.lastIndexOf("/") + 1)} />
                  </AntdSpace>
                )}
