...
export const useHoSoChoCanBoTraKQColumn = (
  ...
  > => {
    return [
      ...
      {
        title: <p style={{ fontSize: 16, textAlign: 'center' }}>Người được ủy quyền</p>,
        key: 'nguoiUyQuyen',
        render: (_, record) => {
          return (
            <div
            ...
            >
              <div style={{ fontWeight: '500', marginBottom: '6px' }}>
                {record.nguoiUyQuyen} {record.soDienThoaiNguoiUyQuyen ? `(${record.soDienThoaiNguoiUyQuyen})`:"" }
              </div>
              {record.soDienThoaiNguoiUyQuyen ? (
                <div
                  style={{
                    fontWeight: '500',
                    display: 'flex',
                  ...
                  title="Số điện thoại"
                  onClick={() =>
                    window.open(`tel:+${record.soDienThoaiNguoiUyQuyen}`)
                  }
                >
                  {' '}
                  <i className="fa-solid fa-phone-volume" style={{ marginRight: 5, color: 'rgba(245, 158, 11, 1)' }}></i> {' '}
                  <span>{record.soDienThoaiNguoiUyQuyen}</span>
                </div>
              ) : null}
              {record.emailNguoiUyQuyen ? (
                <div
                  style={{
                    fontWeight: '500',
                    display: 'flex',
                  ...
                  role="button"
                  onClick={() => window.open(`mailto:${record.emailNguoiUyQuyen}`)}
                >
                  {' '}
                  <MailOutlined title="Email" style={{ marginRight: 5, color: 'rgba(245, 158, 11, 1)' }} />{' '}
                  <span>{record.emailNguoiUyQuyen}</span>
                </div>
              ) : null}
            </div>
          )
      ...
      //             <AntdSpace style={{ fontWeight: '500',textAlign:"center" }} direction="vertical">
      ...
      // },
      {
        title: <p style={{ fontSize: 16, textAlign: 'center' }}>Ghi chú</p>,
        key: "viTriDeHoSo",
        dataIndex: "viTriDeHoSo",
        render: (_, record) => {
          const formattedDateNgayLuuViTriHoSo = record.ngayLuuViTriHoSo
            ? dayjs(record.ngayLuuViTriHoSo).format('DD/MM/YYYY HH:mm:ss')
            : null;
          return (
            <>
              {record.viTriDeHoSo ?
                <div>
                  ...
                  <div style={{ fontWeight: '500' }}>
                    - Ngày ghi chú: {formattedDateNgayLuuViTriHoSo}
                  </div>
                  <div style={{ fontWeight: '500' }}>
                    - Cán bộ thực hiện ghi chú: {record.nguoiLuuViTriHoSo}
                  </div>
                </div>
                : null
              }
      ...
      {
        title: <p style={{ fontSize: 16, textAlign: 'center' }}>Thông tin kết quả</p>,
        key: "ketQua",
        render: (_, record) => {
          const formattedDate = record.ngayXacNhanKetQua
            ? dayjs(record.ngayXacNhanKetQua, 'MM/DD/YYYY HH:mm:ss').format('DD/MM/YYYY')
            : null;
          return (
            <>
              {record.ngayXacNhanKetQua ?
                <div style={{ fontWeight: '500' }}>
                  - Ngày xác nhận: {formattedDate}
                </div> : null
              }
              <div style={{ fontWeight: '500' }}>
