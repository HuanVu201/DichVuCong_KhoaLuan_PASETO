...
export const usePhiLePhiColumn = ({
  dataSource,
  setDataSource,
}: {
  dataSource: IPhiLePhi[];
  setDataSource: React.Dispatch<React.SetStateAction<IPhiLePhi[]>>;
}) => {
  const [_, startTransition] = useTransition();
  const onRowChange = useCallback(
    (value: any, index: number, colName: keyof IPhiLePhi) => {
    ...
    return [
      ...
      },
      {
        ...
        render: (_, record, idx) => {
          return (
            <Select
              options={LOAIPHILEPHI_TABLE_OPTIONS}
              defaultValue={record.loai}
              onChange={(value) => onRowChange(value, idx, "loai")}
            />
          );
        },
      },
      {
        ...
        render: (_, record, idx) => {
          return (
            <InputNumber
              min={0}
              defaultValue={record.soTien}
              formatter={(value: number | undefined) =>
                value ? getCurrency(value) : "0"
              }
              onChange={(value) => onRowChange(value, idx, "soTien")}
            />
          );
        },
      },
      ...
      //     key: "soTienBangChu",
      //     dataIndex: "soTienBangChu",
      //     render: (_, record, idx) => {

      //         return <Input  disabled defaultValue={numberToCurrencyText(record.soTien)} onChange={(value) => onRowChange(value, idx, "soTien")}/>
      //     }
      // },
      {
        title: "Thao tác",
          ...
          return (
            <AntdSpace>
              <Popconfirm
                ...
                onConfirm={() => {
                  setDataSource((curr) =>
                    curr.filter((x) => x.id !== record.id)
                  );
                }}
                okText="Xoá"
                cancelText="Huỷ"
              >
                <DeleteOutlined style={{ color: "tomato" }} title="Xóa dòng" />
              </Popconfirm>
            </AntdSpace>
          );
        },
