...
export const useHoSoQuaHanColumn = (
    ...
    > => {
        return [
            ...
            {
                title: <p style={{ fontSize: 16, textAlign: 'center' }}>Số ngày quá hạn</p>,
                key: "soNgayQuaHan",
                dataIndex: "soNgayQuaHan",
                align: 'center',
                render: (text, record) => {
                    const ngayHienTai = dayjs();
                    const ngayHenTra = dayjs(record.ngayHenTra)
                    const soNgayKhacBiet = ngayHienTai.diff(ngayHenTra, 'day');
                    return (
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', fontWeight: '500' }}>
                            {soNgayKhacBiet + 1}  Ngày
                        </div>
                    )
                },
                width: '10%',
