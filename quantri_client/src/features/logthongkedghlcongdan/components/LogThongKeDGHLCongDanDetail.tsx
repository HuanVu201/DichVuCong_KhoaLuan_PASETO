...
import { GetLogThongKeDGHLCongDan } from "../redux/action"

const options = [
    { label: 'True', value: true as any },
    { label: 'False', value: false as any },
];
export const LogThongKeDanhGiaHaiLongCongDanDetail = () => {
    const dispatch = useAppDispatch()
    const { data: ThongKeDanhGiaHaiLong, } = useAppSelector(state => state.logthongkedghlcongdan)
    ...
    const [form] = Form.useForm<ILogThongKeDGHLCongDan>()
    ...
    const handleCancel = () => {
        form.resetFields();
        // dispatch(resetData)
        ThongKeDanhGiaHaiLongContext.setLogThongKeDGHLCongDanModalVisible(false)
        ThongKeDanhGiaHaiLongContext.setLogThongKeDGHLCongDanId(undefined)
    };
    useEffect(() => {
        if (ThongKeDanhGiaHaiLongContext.LogThongKeDGHLCongDanId) {
            dispatch(GetLogThongKeDGHLCongDan(ThongKeDanhGiaHaiLongContext.LogThongKeDGHLCongDanId))
        }
    }, [ThongKeDanhGiaHaiLongContext.LogThongKeDGHLCongDanId])
    ...
    return (
        <AntdModal title="Chi tiết đánh giá" visible={ThongKeDanhGiaHaiLongContext.LogThongKeDGHLCongDanModalVisible} handlerCancel={handleCancel} footer={null}>
            <Form name='ThongKeDanhGiaHaiLong' layout="vertical" onFinish={onFinish} form={form} requiredMark={ThongKeDanhGiaHaiLongContext.LogThongKeDGHLCongDanId === null} initialValues={{ thuTu: 1 }}>
                <Row gutter={[8, 8]}>
                    <Col md={12} span={24}>
                        <Form.Item
                            label="Mã hồ sơ"
