...
const PhanQuyenQuanTriDonVi = ({
  ...
  const loadRoles = async () => {
    let res = await vaiTroService.SearchQuanTriDonVi({ pageNumber: 10, pageSize: 1 });
  ...
  useEffect(() => {
    (async () => {
      ...
      if (users && users.length == 1) {
        ...
        let user = await userService.GetUserRoles(userId, true);
        ...
        toast.warn("Vui lòng chọn người dùng để phân quyền!")
      }
      setLoading(false);
    })();
  }, [users]);
