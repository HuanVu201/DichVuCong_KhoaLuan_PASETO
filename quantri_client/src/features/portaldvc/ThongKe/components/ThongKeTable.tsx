...
import { ThongKeTongHop } from './ThongKeDetail/TongHop'
...
export const ThongKeTable = ({ searchQuery, useThongKe }: { searchQuery: URLSearchParams, useThongKe:any }) => {
    ...
    const onSelect = (e: any) => {
        if (e == 'xa-phuong')
            toast.info('Chọn đơn vị để thực hiện thống kê!');
    ...
    useEffect(() => {
        ...
        setSearchParams((curr) => ({ ...curr, tuNgay: firstDay, denNgay: lastDay, catalog: catalogSelected || undefined } as any))
    ...
    const [sortByThuTuAndTenThongKe] = useMemo(() => {
        ...
        return [baoCaoDonVis?.sort((a, b) => (a.thuTu || 0) - (b.thuTu || 1))]
    }, [baoCaoDonVis])
    ...
    return (
        ...
        >
            <div className="d-flex flex-wrap selectThongKe">
                <span style={{ fontWeight: 600 }}>Thống kê nhóm đơn vị: </span>
                <AntdSelect options={optionsTest} onSelect={onSelect} style={{ width: '100%' }} defaultValue={""}></AntdSelect>
                {thongKePortalContext.catalogSearchPortal == 'xa-phuong' ?
                    <Form form={form} name="" style={{ marginTop: '15px', flex: 1 }}>
                        <div className='row' >
            ...
            <div className='tableThongKe' style={{ margin: 'auto' }}>
                {/* <ThongKeTab /> */}
                <div style={{marginTop: 30}}>
                    <ThongKeTongHop useThongKe={useThongKe}/>
                </div>
