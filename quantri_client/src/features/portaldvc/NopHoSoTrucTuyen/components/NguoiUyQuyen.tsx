...
export const NguoiUyQuyen = ({form, setDiaBan, diaBan, hideRule}: {hideRule: boolean | undefined; form: FormInstance<IHoSo>; diaBan: DiaBanUyQuyen; setDiaBan: React.Dispatch<React.SetStateAction<DiaBanUyQuyen>> }) => {
    ...
    const soGiayToNguoiUyQuyen = Form.useWatch("soGiayToNguoiUyQuyen", form)
        ...
        if(value){
            ...
            if (currentSelect != "Xa") {
                ...
                if(currentSelect == "Tinh"){
                    ...
                    form.resetFields(["quanHuyenNguoiUyQuyen","xaPhuongNguoiUyQuyen"])
                }
                if(currentSelect == "Huyen"){
                    setDiaBan((curr) => ({...curr, maXa: diabanRes.data.data}))
                    form.resetFields(["xaPhuongNguoiUyQuyen"])
    ...
    return (
        <Row gutter={8}>
            ...
            <Col span={24} lg={6} md={12}>
                <Form.Item name="soGiayToNguoiUyQuyen" label="CCCD/CMND/MĐD:" rules={hideRule ? undefined : [{ required: true, message: "Vui lòng nhập thông tin" }]}>
                    <Input disabled={soGiayToNguoiUyQuyen ? true : false} />
                </Form.Item>
            </Col>
            <Col span={24} lg={6} md={12}>
                <Form.Item name="soDienThoaiNguoiUyQuyen" label="Số điện thoại:" rules={hideRule ? undefined : INPUT_RULES.soDienThoaiChuHoSo}>
                    <Input minLength={10} maxLength={13}/>
                </Form.Item>
            </Col>
            <Col span={24} lg={6} md={12}>
                <Form.Item name="emailNguoiUyQuyen" label="Email:" rules={INPUT_RULES.emailChuHoSo}>
                    <Input />
                </Form.Item>
            </Col>
            <Col span={24} lg={6} md={12}>
