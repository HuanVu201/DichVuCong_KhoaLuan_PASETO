...
import { renderDVCRouteParams } from "../data/constant";
import { QUAN_HUYEN, SO_BAN_NGANH, XA_PHUONG } from "./SearchThuTuc";
import { getCurrency } from "@/utils";

export const DanhSachThuTucsPortal = () => {
  const {
    datas: thuTucs,
    count,
  ...
  return (
    <div>
      <Spin spinning={loadingThuTucs}>
        {count && count > 0 ? (
          <div>
            <span className="countDanhMucTT" style={{ color: '#b00600', fontWeight: '400' }}>Có {getCurrency(count, ".")} thủ tục</span>
            {/* <h3>Dịch vụ công trực tuyến của Sở Nội vụ</h3> */}
            {thuTucs?.map((thuTuc, index) => (
              <CardThuTucPortal index={index} showIndex={true} thuTuc={thuTuc} key={index} />
            ))}
