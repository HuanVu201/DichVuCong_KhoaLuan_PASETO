...
export const useGiayToSoHoaTrongKhoColumn = () => {
                ...
                render: (_, record) => {
                    return <div style={{ display: 'flex', justifyContent: 'center', gap: 5, flexWrap: 'wrap' }}>
                        <EyeOutlined style={{ color: "cornflowerblue" }} title="Xem chi tiết/Sửa" onClick={() => {
                            khoTaiLieuDienTuContext.setGiayToSoHoaItem(record)
                            khoTaiLieuDienTuContext.setTaiLieuDetailModalVisible(true)
                        }} />
                        <ShareAltOutlined style={{ color: "orange" }} title="Chia sẻ giấy tờ" onClick={() => {
                            khoTaiLieuDienTuContext.setGiayToSoHoaItem(record)
                            khoTaiLieuDienTuContext.setChiaSeTaiLieuModalVisible(true)
                        }} />
                        <CloudUploadOutlined style={{ color: "cornflowerblue" }} title="Phiên bản chỉnh sửa" onClick={() => {
                            khoTaiLieuDienTuContext.setGiayToSoHoaItem(record)
                            khoTaiLieuDienTuContext.setDetailVersionModalVisible(true)
                        }} />

                        <Popconfirm
                            title='Xoá?'
                            onConfirm={() => {
                                khoTaiLieuDienTuContext.setGiayToXoaKhoiKhoId(record.id)
                            }}
                            okText='Xoá'
                            cancelText='Huỷ'
