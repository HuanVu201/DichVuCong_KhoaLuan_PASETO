      ...
      <div style={{ width: "100%", height: "100%", textAlign: "center" }}>
        <AntdSpace direction="vertical">
          {data.tenNhomNguoiDung ? (
            <Title level={3}>{data.tenNhomNguoiDung}</Title>
          ) : null}
          {data.tenBuocXuLy ? (
          ...
          <Text style={{fontSize:"1.25rem"}}>Trực tiếp {data.thoiGianXuLy} (giờ) | Trực tuyến {data.thoiGianThucHienTrucTuyen} (giờ)</Text>
          {data.tenTrangThaiHoSo ? (
            <Text type="warning" style={{fontSize:"1.25rem"}}>{data.tenTrangThaiHoSo}</Text>
          ) : null}
        </AntdSpace>
      </div>
      <Handle
