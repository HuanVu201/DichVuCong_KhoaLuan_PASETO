...
const ChiTietQuyTrinhQuanTriDonVi = (props: CustomReactFlowProps) => {
    ...
    const nodeTypes = useMemo(() => ({ 
        customNode: (props: Node<IQuyTrinhXuLy>) => <CustomNode  {...props}/>,
        startNode: (props: Node<IQuyTrinhXuLy>) => <StartNode {...props}/>, 
        endNode: EndNode }), []);
    ...
    useEffect(() => {
        if(truongHopThuTuc){
            ...
            if(nodes.length == 0){ // đây là dữ liệu đồng bộ nên chưa có các node mặc định
                ...
                const quyTrinhStart : Pick<IQuyTrinhXuLy, "truongHopId" | "tenBuocXuLy" | "thoiGianXuLy" | "loaiThoiGian" | "id"| "maTrangThaiHoSo" | "thoiGianThucHienTrucTuyen"> = {
                    id: quyTrinhStartId,
    ...
    return <>
    <AntdModal visible={true} title="Chi tiết quy trình" fullsize footer={null} handlerCancel={handlerCancel} afterOpenChange={(open) => {
...
const ChiTietQuyTrinhQuanTriDonViModal = (props: any) => {
    return (
        <ReactFlowProvider>
            <ChiTietQuyTrinhQuanTriDonVi {...props} />
        </ReactFlowProvider>
      );
}
export default ChiTietQuyTrinhQuanTriDonViModal