...
import { DatLaiNgayHenTraParams, DatLaiQuyTrinhXuLyParams, adminHoSoApi, AdminUpdateHoSoParams, AdminKetThucXuLyNhieuHoSosParams } from "../services";
...
export const AdminKetThucXuLyNhieuHoSo = createAsyncThunk<IResult<any>, AdminKetThucXuLyNhieuHoSosParams, { rejectValue: IResult<any> }>("AdminKetThucXuLyNhieuHoSo", async (data, thunkApi) => {
    try {
        const res = await adminHoSoApi.AdminKetThucXuLyNhieuHoSo(data);
        // if (res.status === 200) {
        //     thunkApi.dispatch(SearchHoSo({ reFetch: true }))
        // }
        return res.data
