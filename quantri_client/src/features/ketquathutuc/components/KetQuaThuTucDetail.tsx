...
export const KetQuaThuTucDetail = ({ setSearchParams }: { setSearchParams: React.Dispatch<React.SetStateAction<ISearchKetQuaThuTuc>> }) => {
    ...
    const { datas: thuTucs, loading } = useAppSelector((state) => state.thutuc);
    useEffect(() => {
        dispatch(
            SearchThuTuc({
                pageNumber: 1,
    ...
    const onFinish = async () => {
        ...
        setSearchParams(cur => ({ ...cur, thuTucId: formData.maTTHC }))
        resetState()
    }
    useEffect(() => {
        if (thuTucContext.thuTucId) {
            form.setFieldValue("maTTHC", thuTucContext.thuTucId)
        }
    }, [
        ...
        <Spin spinning={loading}>
            <AntdModal title="Thêm mới kết quả thủ tục" visible={true} handlerCancel={handleCancel} footer={<>
                ...
                <Form name='ketquathutuc' layout="vertical" form={form} requiredMark={ketQuaThuTucContext.maKetQuaThuTuc !== null}
                    ...
                    <Row gutter={[8, 8]}>
                        <Col md={24} span={24} >
                            <Form.Item label="Thủ tục" name="maTTHC" required rules={[{ required: true, message: 'Không được để trống' }]}>
                                <AntdSelect
                                    generateOptions={{
                                        model: thuTucs,
                                        label: "tenTTHC",
