...
import { SuaBienLaiThanhToanModal } from "./SuaBienLaiThanhToanModal";

const TYPE_OPTIONS: SelectProps["options"] = [
  { label: "Đơn vị", value: "don-vi" },
  { label: "Nhóm", value: "nhom" },
];
const ILIS_OPTIONS: SelectProps["options"] = [
  { label: "VBDLIS", value: "VBDLIS" },
  { label: "ILIS", value: "ILIS" },
];
const coCauHinhVnPost = import.meta.env.VITE_ENABLE_VNPOST ?? false
const coCauHinhILIS = import.meta.env.VITE_ENABLE_CAUHINH_TNMT ?? false
const ThongTin = () => {
  ...
  console.log(import.meta.env.VITE_ENABLE_VNPOST)
  useEffect(() => {
    (async () => {
    ...
    <>
      ...
      >
        <Row gutter={[8, 8]}>
          ...
          <Col md={12} span={24}>
            <Form.Item label="Thông tin tọa độ" name="coordinates">
              <Input disabled={isDisabled} />
            </Form.Item>
          </Col>
          ...
          <Row style={{ display: 'flex', width: '100%', gap: 10 }}>
            ...
            {loaiBienLaiThanhToan == 'local'
              ...
              <Col style={{ flex: 1, display: 'flex' }}>
                ...
                <EditOutlined title="Xem chi tiết/Sửa"
                  hidden={isDisabled ? true : false}
                  style={{ color: "cornflowerblue", padding: '0 10px', fontSize: 20 }}
                  onClick={() => { suaCauHinhBienLaiThanhToan() }} />
              </Col>
        ...
        <Row gutter={[8, 8]}>
          <Col md={12} span={24} hidden={!coCauHinhVnPost}>
            <Form.Item label="Cấu hình bưu điện" name="cauHinhBuuDien">
              <TextArea disabled={isDisabled} />
            </Form.Item>
          </Col>
          <Col md={12} span={24} hidden={!coCauHinhILIS}>
            <Form.Item label="Loại liên thông TNMT" name="lienThongTNMT">
              <AntdSelect
                options={ILIS_OPTIONS}
                disabled={isDisabled}
              />
            </Form.Item>
          </Col>
