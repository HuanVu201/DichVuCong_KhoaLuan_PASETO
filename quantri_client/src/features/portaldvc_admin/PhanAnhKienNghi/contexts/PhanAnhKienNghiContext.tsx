...
export interface IPhanAnhKienNghiContext {
  ...
  xoaPAKNId: string | undefined;
  setXoaPAKNId: React.Dispatch<React.SetStateAction<string | undefined>>;
...
export const PhanAnhKienNghiProvider = ({ children }: IWithChildren) => {
  ...
  const [xoaPAKNId, setXoaPAKNId] = useState<string>();
  const [PhanAnhKienNghiVisible, setPhanAnhKienNghiVisible] = useState<boolean>(false)
  return (
    <PhanAnhKienNghiContext.Provider
      value={{ PhanAnhKienNghiId, setPhanAnhKienNghiId, PhanAnhKienNghiVisible, setPhanAnhKienNghiVisible, xoaPAKNId, setXoaPAKNId }}
    >
      {children}
    </PhanAnhKienNghiContext.Provider>
  );
