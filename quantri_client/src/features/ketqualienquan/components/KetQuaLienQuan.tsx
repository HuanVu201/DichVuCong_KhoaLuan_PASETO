...
const KetQuaLienQuan = ({maHoSo, readOnlyMode}: {maHoSo: string; readOnlyMode?: boolean}) => {
    const [searchParams, setSearchParams] = useState<ISearchKetQuaLienQuan>({pageNumber: 1, pageSize: 50, maHoSo})
    const ketQuaLienQuanContext = useKetQuaLienQuanContext()
    const columns = useColumn({searchParams, readOnlyMode})

    return <>
        {readOnlyMode ? null : <KetQuaLienQuanSearch setSearchParams={setSearchParams}/>}
        <AntdTable
