...
export const PhiLePhiDetail = ({
  setSearchParams,
}: {
  setSearchParams: React.Dispatch<React.SetStateAction<ISearchPhiLePhi>>;
}) => {
  const dispatch = useAppDispatch();
  const { data: phiLePhi, datas: phiLePhis } = useAppSelector(
    (state) => state.phiLePhi
  );
  ...
  const onFinish = async () => {
    ...
    if (phiiLePhiContext?.maPhiLePhi) {
      await dispatch(
        UpdatePhiLePhi({
          id: phiiLePhiContext.maPhiLePhi,
          data: { ...formData },
        })
      ).unwrap();
    } else {
      if (thuTucContext.thuTucId) {
        await dispatch(
          AddPhiLePhi({ ...formData, thuTucId: thuTucContext.thuTucId })
        ).unwrap();
      }
    }
  ...
  return (
    ...
    >
      <Form
        ...
        onFinish={onFinish}
        form={form}
        requiredMark={phiiLePhiContext.maPhiLePhi !== null}
      >
        <Row gutter={[8, 8]}>
          <Col md={12} span={24}>
          ...
          <Col md={12} span={24}>
            <Form.Item label="Số tiền (VNĐ)" name="soTien">
