...
export const SearchGTSHOutsideKhoTaiLieu =
    createAsyncThunk<IPaginationResponse<IGiayToSoHoa[]>, ISearchGiayToSoHoa>("SearchGTSHOutsideKhoTaiLieu", async (params, thunkApi) => {
        try {
            const res = await giayToSoHoaApi.SearchGTSHOutsideKhoTaiLieu(params)
            return res.data
        } catch (error) {
            return thunkApi.rejectWithValue(error)
        }
...
export const UpdateGTSHKhoTaiLieu = createAsyncThunk("UpdateGTSHKhoTaiLieu", async (data: IOmitUpdate<IGiayToSoHoa>, thunkApi) => {
    try {
        const res = await giayToSoHoaApi.UpdateGTSHKhoTaiLieu(data);
        // if (res.status === 200) {
        //     thunkApi.dispatch(SearchGiayToSoHoa({ reFetch: true }))
        // }
        return res.data
