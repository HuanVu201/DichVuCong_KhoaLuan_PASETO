...
import { useHoSoColumn } from "../../hooks/useHoSoColumn"
import axiosInstance from "@/lib/axios"
import axios from "axios"
import { CURRENTTIME, FORMAT_DATE, HOST_PATH } from "@/data"
import { HoSoDetail } from "./HoSoDetail"
import { HoSoLienThongQLVBSearch } from "./HoSoSearch"
import { getCurrencyThongKe } from "@/utils"

const HoSoTable = () => {
    const [hoSoData, setHoSoData] = useState<ILienThongQLVB[]>()
    ...
    const { columns } = useHoSoColumn({ pageNumber: 1, pageSize: 50 })
    ...
    const searchGoiTin = async () => {
        ...
        try {
            ...
            if (response.data) {
                setHoSoData(response.data.data)
            }
        ...
        <>
            <AntdSpace direction="vertical" style={{ width: "100%" }}>
                ...
                >
                    ...
                    <AntdDivider />
                    <HoSoLienThongQLVBSearch setSearchParams={setSearchParams} />
                    {/* <HoSoSearch setSearchParams={setSearchParams} /> */}
                    <div style={{ marginBottom: 8 }}>

                        <p><b>Số lượng: {getCurrencyThongKe(hoSoData?.length || 0)} hồ sơ</b></p>

                    </div>
                    <AntdTable
                        columns={columns as any}
                        dataSource={hoSoData as any}
                        pagination={{
                            total: hoSoData?.length
                        }}
                        searchParams={qlvbContext.searchParams as any}
                        setSearchParams={qlvbContext.setSearchParams as any}
                        onSearch={() => { }}
                    />
                </Spin>
            </AntdSpace>
...
const HoSoLienThongQLVBTableWrapper = () => (<LienThongQLVBProvider>
    <HoSoTable />
</LienThongQLVBProvider>)
export default HoSoLienThongQLVBTableWrapper