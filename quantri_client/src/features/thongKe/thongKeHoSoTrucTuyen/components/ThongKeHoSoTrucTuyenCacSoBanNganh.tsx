...
import {
  useThongKeHoSoTrucTuyenCacSoBanNganhContext,
...
const items: MenuProps["items"] = [
  {
    label: (
      <button
        onClick={() => {
          export2Word(
            "Bảng thống kê tiếp nhận hồ sơ trực tuyến các sở, ban, ngành tỉnh ", true,
          );
        }}
  ...
  },
  {
    label: (
      <button
        ...
        onClick={() =>
          downloadPhieuExcel(
            "Bảng thống kê tiếp nhận hồ sơ trực tuyến các sở, ban, ngành tỉnh"
          )
        }
      >
...
const ThongKeHoSoTrucTuyenCacSoBanNganh = () => {
  ...
  useEffect(() => {
    if (thongKeHoSoContext.search.tuNgay && thongKeHoSoContext.search.denNgay) {
      (async () => {
        setLoading(true)
        const res = await dispatch(SearchTkHoSoTrucTuyenCacSoBanNganh(thongKeHoSoContext.search)).unwrap();
  ...
  thongKeDatas?.forEach((item: any, index) => {
    ...
    dataHtmlForExcel +=
      "<tr>" +
      "<td style=' vertical-align: middle; padding: 5px; border: 1px solid #333; text-align: center; '>" +
      "<strong>" +
      Math.abs(index + 1) +
      "</strong>" +
      "</td >" +
      ...
      getCurrencyThongKe(item.tongBCCI) +
      "</td>" +
      "<td rowspan='3' style='vertical-align: middle; padding: 5px; border: 1px solid #333; text-align: center; ' >" +
      (item.tongTrucTuyen / (item.tongTrucTiep + item.tongTrucTuyen) > 0
        ? "<strong>" +
        Math.round((item.tongTrucTuyen / (item.tongTrucTiep + item.tongTrucTuyen)) * 100 * 100) / 100 +
        "%</strong>"
      ...
      (item.tongToanTrinhTrucTuyen / (item.tongToanTrinhTrucTuyen + item.tongToanTrinhTrucTiep) > 0
        ...
        : "<strong></strong>") +
      "</td>" +
      "<td rowspan='3' style='vertical-align: middle; padding: 5px; border: 1px solid #333; text-align: right; ' >" +
      "<strong></strong>" +
      "</td>" +
      "<td rowspan='3' style='vertical-align: middle; padding: 5px; border: 1px solid #333; text-align: right; ' >" +
      "<strong></strong>" +
      ...
      "</tr >" +
      "<tr>" +
      "<td style='vertical-align: middle; padding: 5px; border: 1px solid #333; text-align: right; ' ></td>" +
      "<td style='vertical-align: middle; padding: 5px; border: 1px solid #333; text-align: left; ' >" +
      "DVC trực tuyến một phần" +
      "</td>" +
      "<td style='vertical-align: middle; padding: 5px; border: 1px solid #333; text-align: right; ' >" +
  ...
  return (
    <div className="thongKeSwapper table-responsive">
      <div className="title" style={{ fontSize: 18, fontWeight: 600 }}>
        Bảng thống kê tiếp nhận hồ sơ trực tuyến của các Sở, ban, ngành cấp tỉnh
      </div>
            ...
            >
              ...
              <tr>
                ...
                >
                  ...
                  <br />
                  <strong>
                    BẢNG THỐNG KÊ TIẾP NHẬN HỒ SƠ TRỰC TUYẾN CỦA CÁC SỞ, BAN, NGÀNH
                    CẤP TỈNH{" "}
                  </strong>
                  <br />
                  <strong
              ...
              <tbody id="data">
                {thongKeDatas && thongKeDatas.length > 0
                  ...
                  : null}
                <CountDataThongKeHoSoTrucTuyenCacSoBanNganh data={thongKeDatas} />
              </tbody>
            </table>
          </div>
        </Spin>
...
export const CountDataThongKeHoSoTrucTuyenCacSoBanNganh = ({
  data,
}: {
