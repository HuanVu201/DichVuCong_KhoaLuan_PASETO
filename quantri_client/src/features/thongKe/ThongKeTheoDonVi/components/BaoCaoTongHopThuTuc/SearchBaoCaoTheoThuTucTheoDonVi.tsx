...
export const SearchBaoCaoTheoThuTucTheoDonVi = ({
  setSearchParams,
  resetSearchParams,
  loaiDonViThongKe,
  setThongKeToanBo,
  ...
  useEffect(() => {
    if (userData && (form.getFieldValue("MaDinhDanhCha") && coCauToChucs)) {
      var groupCode = coCauToChucs?.find(x => x.maDinhDanh == (form.getFieldValue("MaDinhDanhCha") || maDinhDanh))
      dispatch(SearchThuTuc({ pageNumber: 1, pageSize: 10000, donVi: groupCode?.groupCode, reFetch: true }));
    }
  }, [userData, form.getFieldValue("MaDinhDanhCha"), coCauToChucs])
  useEffect(() => {
    if (auth) {
      ...
      if (tuNgayData && denNgayData) {
        ...
        if (thongKeToanDonVi == "toanbo") {
          ...
          setSearchParams((curr: any) => ({
            ...
            maTTHC: form.getFieldValue("maTTHC"),
    ...
    >
      ...
      >
        ...
        <Row gutter={[8, 8]} style={{ display: 'flex' }}>
          ...
          <Col md={6} span={12}>
            <Form.Item
              label="Có phát sinh hồ sơ"
              name="coPhatSinhHoSo"
            >
              <Radio.Group
                options={coPhatSinhHoSosOptions as any}
                onChange={((e: any) => {
                  setCoPhatSinhHoSo(e.target.value)
                })}
          ...
          <Col md={6} span={12} >
            ...
            >
              <Radio.Group
                options={coPhatSinhHoSosOptions}
                defaultValue={undefined}
                optionType="button"
                buttonStyle="solid"
              />
