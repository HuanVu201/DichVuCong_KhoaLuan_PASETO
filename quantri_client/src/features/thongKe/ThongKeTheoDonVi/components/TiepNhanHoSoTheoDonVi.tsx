...
const items: MenuProps["items"] = [
  {
    label: (
      <button
        ...
        onClick={() => downloadPhieuExcel("Thống kê tiếp nhận hồ sơ theo đơn vị")}
      >
        ...
        >
          {(item.daXuLyDungHanVaTraLai + item.daXuLyTruocHan) /
            ...
            ? (Math.round(
              ...
              100).toFixed(2).replace('.', ',').replace(/\.00$/, '') +
            "%"
            : "0"}
        </td>
    ...
    <>
      <div className="thongKeSwapper">
        <div className="title" style={{ fontSize: 18, fontWeight: 600 }}>
          Bảng thống kê chi tiết việc tiếp nhận và trả kết quả
        </div>
        <SearchBaoCaoTongHop
        ...
        <XuatTiepNhanHoSoTheoDonViModal data={data}
...
export const CountTiepNhanHoSoTheoDonVi = ({
  data,
}: {
  ...
  return (
    <tr>
      ...
      >
        {(daXuLyDungHanVaTraLaiTotal + daXuLyTruocHanTotal) /
          (daXuLyDungHanVaTraLaiTotal + daXuLyTruocHanTotal + daXuLyQuaHanTotal)
          ? Math.round(
            ((daXuLyDungHanVaTraLaiTotal + daXuLyTruocHanTotal) /
              (daXuLyDungHanVaTraLaiTotal +
                daXuLyTruocHanTotal +
                daXuLyQuaHanTotal)) *
            100 *
            100
