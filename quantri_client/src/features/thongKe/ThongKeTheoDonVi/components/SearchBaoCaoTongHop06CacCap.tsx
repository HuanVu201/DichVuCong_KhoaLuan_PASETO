...
export const SearchBaoCaoTongHop06CacCap = ({
  setSearchParams,
  resetSearchParams,
  loaiDonViThongKe,
  onFinish,
  ...
  useEffect(() => {
    var tmpDonVi: any = coCauToChucs?.find(item => item.maDinhDanh == userData?.maDinhDanh);
    var tmpCatalog = tmpDonVi?.catalog ?? undefined;
    const now = new Date();
  ...
  const setSearchValue = () => {
    ...
    var tmpDonVi: any = coCauToChucs?.find(item => item.maDinhDanh == donViData);
    var tmpCatalog = tmpDonVi?.catalog ?? undefined;
      ...
      if (tuNgayData && denNgayData) {
        ...
        if (thongKeToanDonVi == "toanbo") {
          setSearchParams((curr: any) => ({
            ...
            tuNgay: tuNgayData,
            denNgay: denNgayData,
            catalog: donViData ? tmpCatalog : form.getFieldValue("catalog"),
        ...
        <Row gutter={[8, 8]}>
          ...
          <Col md={8}>
            <Form.Item
              ...
              style={{ display: loaiDonViThongKe ? "none" : "" }}
            >
              <Radio.Group
