...
export const SearchSoTheoDoiHoSo = ({
  setSearchParams,
  resetSearchParams,
  onFinish,
  searchParams,
  ...
  const { datas: users, loading: loadingUser } = useAppSelector((state) => state.user);
  ...
  const selectedGroups = Form.useWatch("groupCode", form);
  useEffect(() => {
  ...
  useEffect(() => {
    if (thongKeToanHeThong && selectedGroups) {
      form.setFieldValue("nguoiNhanHoSo", undefined);
      dispatch(SearchUser({
        typeUser: "CanBo",

        officeCode: selectedGroups
      }))
    }
  }, [selectedGroups, thongKeToanHeThong])
  return (
    <Form
      name="ThongKeSearch"
      layout="horizontal"
      onFieldsChange={(changedFields, allFields) => {
        if (
          ...
          form.setFieldValue("groupCode", undefined);
          form.setFieldValue("nguoiNhanHoSo", undefined);
        }
    ...
    >
      ...
      <Row gutter={[8, 0]}>
        <Col md={8}>
          <Form.Item label="Người tiếp nhận" name="nguoiNhanHoSo">
            <AntdSelect generateOptions={{
              model: users,
              label: "fullName",
              value: "id"

            }} style={{ width: '100%' }} loading={loadingUser} />
          </Form.Item>
        </Col>
        <Col md={8}>
