...
export const DanhSachTaiKhoanSearch = ({
  ...
  const resetSearchParams = useCallback(() => {
    ...
    setSearchParams({ reFetch: true,  officeCode: user?.officeCode});
    form.resetFields();
    form.setFieldValue('officeCode', user?.officeCode)
  }, []);

  useEffect(() => {
    (async () => {
      await dispatch(SearchCoCauToChuc({
        ...searchDonViParams,
        // orderBy: ["GroupOrder", "GroupCode"],
        officeCode: user?.officeCode || undefined,
        // getAllChildren: true,
      }));
    })();
  }, [searchDonViParams]);
