...
export async function generateDocxWithImagesPhieuHoSo(fileUrl: string, data: any, saveAsFileName: `${string}.docx`, callback?: (blobData: any) => Promise<void>, save: boolean = true, imageSize: [number, number] = [100, 100], docOutType: "blob" | "string" | "base64" = "blob") {
  const blob = await fileApi.GetFileByte({ path: fileUrl });
  const blobUrl = window.URL.createObjectURL(blob.data);
  if (!blobUrl) {
    return
...
export const fillEformData = async (fileUrl: string, data: any, saveAsFileName: string, saveToFolder?: string, onDone?: (uploadedFileName: string) => void, flag: "word" | "word2pdf" | "saveToTable" | "saveAsPdf" = "word") => {
  ...
  loadFilePizzip(blobUrl, async function (error, content) {
    ...
    else if (flag == "saveAsPdf") {
      await downloadPDf(doc, fileUrl)
    }
  })
}
...
export const downloadPDf = async (doc: Docxtemplate<PizZip>, fileUrl: string) => {
  const out = doc.getZip().generate({
    type: "base64",
    mimeType:
      'application/vnd.openxmlformats-officedocument.wordprocessingml.document',
