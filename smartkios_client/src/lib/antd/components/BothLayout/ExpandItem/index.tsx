...
function ExpandItem() {
    ...
    return (
        <div className='expandItem' style={{ zIndex: 0 }}>
            ...
            <div className='expandItem_support' style={{ zIndex: 0, bottom: CHATBOT_WIDTH + 30 }} id="socialWidgets">
                {showGoToTop && (
...

                {
                    ...
                    <Link className='hcc-DvclinkDvcQG connectSocial' id='dvcQGBlock' to={linkDvcQG} target="_blank"
                    ...
                    >
                        ...
                        <img className="iconSocial" id='dvcQG' src='/images/dvcLogo.png' alt="Cổng Dịch vụ công Quốc gia" />
                    </Link>
                }
                {
                    linkFacebook && useLinkFacebook &&
                    <Link className='hcc-facebook connectSocial' id='facebookBlock' to={linkFacebook} target="_blank"
                        onMouseOver={(e: any) => MouseEnterHandler(e.target.id)}
                        onMouseOut={(e: any) => MouseLeaveHandler(e.target.id)}
                    >
                        <div id="facebookBlockAlert" className="blockAlert">
                            Facebook
                        </div>
                        <img className="iconSocial" id='facebook' src='/images/fbThanhHoa.png' alt="" />
                    </Link>
                }
                {
                    linkZalo && useLinkZalo &&
                    <Link className='hcc-zalo connectSocial' id='zaloBlock' to={linkZalo} target="_blank"
                        onMouseOver={(e: any) => MouseEnterHandler(e.target.id)}
                        onMouseOut={(e: any) => MouseLeaveHandler(e.target.id)}
                    >
                        ...
                        <img className="iconSocial" id='zalo' src='/images/zaloThanhHoa.png' />
                    </Link>

                }
                {chatBotConfig && chatBotConfig?.show ? <div id="chatbot">

                    <td-chatbot logo="dichvucong" line1={chatBotConfig.line1 || ""} line2={chatBotConfig.line2 || ""}
                        welcomemessage={chatBotConfig.welcomeMessage} bot_id={chatBotConfig.botId}
                        inputplaceholder={chatBotConfig.inputplaceholder} zIndex={chatBotConfig.zIndex} ref={chatbotRef}>
