...
import { SearchYeuCauThanhToanPortal } from "@/features/portaldvc/ThanhToan/redux/action";
  ...
  const handleSearch = async (maHoSo: string) => {
    if (maHoSo) {
      ...
      if (!yeuCauThanhToan.data)
        toast.warning("Không có thông tin yêu cầu thanh toán");
      await dispatch(GetHoSoByMa(maHoSo));
    } else {
      ...
      <div className="search-hoso w-100">
        <div className="searchBlock form-left">
          ...
          <div className="thanhToan-footer d-flex justify-content-center align-items-center">
            <Button htmlType="submit" className="form-control btnTimKiemHoSo">Thực hiện</Button>
          </div>
        </div>
