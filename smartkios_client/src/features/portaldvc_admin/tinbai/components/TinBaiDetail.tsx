...
export const TinBaiDetail = () => {
    ...
    const onFinish = async () => {
        ...
        if (tinBaiContext?.maTinBai) {
            dispatch(UpdateTinBai({ id: tinBaiContext.maTinBai, data: { ...formData, noiDung } }))
        }
        else {
            dispatch(AddTinBai({ ...formData }))
        }
        handleCancel()
    }
    ...
    return (
        <AntdModal title="Thêm mới tin bài" visible={tinBaiContext.maTinBaiModalVisible} handlerCancel={handleCancel} footer={null} fullsizeScrollable>
