...
export interface KySoChungThucProps extends BaseActionProps{
    actionName : string
}
// sử dụng comp này bọc ngoài do không thể sử dụng customhook, và các hook bth ở trong useMemo, useEffect
export const KySoChungThucWrapper = (props: KySoChungThucProps) => {
    
    return <KySoChungThuc {...props}></KySoChungThuc>
}

const KySoChungThuc = ({actionName, colorCode, iconName}: KySoChungThucProps) => {
    const buttonActionContext = useButtonActionContext()
    const onClick = () => {
        if(buttonActionContext.selectedHoSos.length){
            buttonActionContext.setKySoChungThucModalVisible(true)
        }else {
