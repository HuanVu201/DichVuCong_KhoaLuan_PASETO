...
export interface SuaHoSoProps extends BaseActionProps{
    actionName : string
}
// sử dụng comp này bọc ngoài do không thể sử dụng customhook, và các hook bth ở trong useMemo, useEffect
export const SuaHoSoWrapper = (props: SuaHoSoProps) => {
    
    return <SuaHoSo {...props}></SuaHoSo>
}

const SuaHoSo = ({actionName, colorCode, iconName}: SuaHoSoProps) => {
    const buttonActionContext = useButtonActionContext()
    const onClick = () => {
        if(buttonActionContext.selectedHoSos.length){
            buttonActionContext.setSuaHoSoModalVisible(true)
        }else {
