...
export interface ThayDoiTruongHopXuLyProps extends BaseActionProps{
    actionName : string
}
// sử dụng comp này bọc ngoài do không thể sử dụng customhook, và các hook bth ở trong useMemo, useEffect
export const ThayDoiTruongHopXuLyWrapper = (props: ThayDoiTruongHopXuLyProps) => {
    
    return <ThayDoiTruongHopXuLy {...props}></ThayDoiTruongHopXuLy>
}

const ThayDoiTruongHopXuLy = ({actionName, colorCode, iconName}: ThayDoiTruongHopXuLyProps) => {
    const buttonActionContext = useButtonActionContext()
    const onClick = () => {
        if(buttonActionContext.selectedHoSos.length){
            buttonActionContext.setThayDoiTruongHopXuLyModalVisible(true)
        }else {
