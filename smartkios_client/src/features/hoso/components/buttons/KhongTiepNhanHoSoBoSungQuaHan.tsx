...
export interface KhongTiepNhanHoSoBoSungQuaHanProps extends BaseActionProps {
    actionName : string
}
// sử dụng comp này bọc ngoài do không thể sử dụng customhook, và các hook bth ở trong useMemo, useEffect
export const KhongTiepNhanHoSoBoSungQuaHanWrapper = (props: KhongTiepNhanHoSoBoSungQuaHanProps) => {
    
    return <KhongTiepNhanHoSoBoSungQuaHan {...props}></KhongTiepNhanHoSoBoSungQuaHan>
}

const KhongTiepNhanHoSoBoSungQuaHan = ({actionName, colorCode, iconName}: KhongTiepNhanHoSoBoSungQuaHanProps) => {
    const buttonActionContext = useButtonActionContext()
    const onClick = () => {
        if(buttonActionContext.selectedHoSos.length){
            buttonActionContext.setKhongTiepNhanHoSoBoSungQuaHanModalVisible(true)
        }else {
