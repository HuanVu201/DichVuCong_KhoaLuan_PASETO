...
export interface YeuCauCongDanBoSungHoSoProps  extends BaseActionProps{
    actionName : string
}
// sử dụng comp này bọc ngoài do không thể sử dụng customhook, và các hook bth ở trong useMemo, useEffect
export const YeuCauCongDanBoSungHoSoWrapper = (props: YeuCauCongDanBoSungHoSoProps) => {
    
    return <YeuCauCongDanBoSungHoSo {...props}></YeuCauCongDanBoSungHoSo>
}

const YeuCauCongDanBoSungHoSo = ({actionName, colorCode, iconName}: YeuCauCongDanBoSungHoSoProps) => {
    const buttonActionContext = useButtonActionContext()
    const onClick = () => {
        if(buttonActionContext.selectedHoSos.length){
            buttonActionContext.setYeuCauCongDanBoSungHoSoModalVisible(true)
        }else {
