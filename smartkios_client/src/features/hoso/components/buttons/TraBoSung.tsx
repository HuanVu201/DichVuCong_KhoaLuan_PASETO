...
export interface TraBoSungProps extends BaseActionProps{
    actionName : string
}
// sử dụng comp này bọc ngoài do không thể sử dụng customhook, và các hook bth ở trong useMemo, useEffect
export const TraBoSungWrapper = (props: TraBoSungProps) => {
    
    return <TraBoSung {...props}></TraBoSung>
}

const TraBoSung = ({actionName, colorCode, iconName}: TraBoSungProps) => {
    const buttonActionContext = useButtonActionContext()
    const onClick = () => {
        if(buttonActionContext.selectedHoSos.length){
            buttonActionContext.setTraBoSungModalVisible(true)
        }else {
