...
export interface CapNhatKetQuaXuLyHoSoProps extends BaseActionProps{
    actionName : string
}
// sử dụng comp này bọc ngoài do không thể sử dụng customhook, và các hook bth ở trong useMemo, useEffect
export const CapNhatKetQuaXuLyHoSoWrapper = (props: CapNhatKetQuaXuLyHoSoProps) => {
    
    return <CapNhatKetQuaXuLyHoSo {...props}></CapNhatKetQuaXuLyHoSo>
}

const CapNhatKetQuaXuLyHoSo = ({actionName, colorCode, iconName}: CapNhatKetQuaXuLyHoSoProps) => {
    const buttonActionContext = useButtonActionContext()
    const onClick = () => {
        if(buttonActionContext.selectedHoSos.length){
            buttonActionContext.setCapNhatKetQuaXuLyHoSoModalVisible(true)
        }else {
