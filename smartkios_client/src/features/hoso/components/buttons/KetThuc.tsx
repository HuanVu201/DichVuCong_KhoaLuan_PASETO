...
export interface KetThucProps extends BaseActionProps{
    actionName : string
}
// sử dụng comp này bọc ngoài do không thể sử dụng customhook, và các hook bth ở trong useMemo, useEffect
export const KetThucWrapper = (props: KetThucProps) => {
    
    return <KetThuc {...props}></KetThuc>
}

const KetThuc = ({actionName, colorCode, iconName}: KetThucProps) => {
    const buttonActionContext = useButtonActionContext()
    const onClick = () => {
        if(buttonActionContext.selectedHoSos.length){
            buttonActionContext.setKetThucModalVisible(true)
        }else {
