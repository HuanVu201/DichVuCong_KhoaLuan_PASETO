...
export interface YeuCauMotCuaBoSungProps extends BaseActionProps {
    actionName : string
}
// sử dụng comp này bọc ngoài do không thể sử dụng customhook, và các hook bth ở trong useMemo, useEffect
export const YeuCauMotCuaBoSungWrapper = (props: YeuCauMotCuaBoSungProps) => {
    
    return <YeuCauMotCuaBoSung {...props}></YeuCauMotCuaBoSung>
}

const YeuCauMotCuaBoSung = ({actionName, colorCode, iconName}: YeuCauMotCuaBoSungProps) => {
    const buttonActionContext = useButtonActionContext()
    const onClick = () => {
        if(buttonActionContext.selectedHoSos.length){
            buttonActionContext.setYeuCauMotCuaBoSungModalVisible(true)
        }else {
