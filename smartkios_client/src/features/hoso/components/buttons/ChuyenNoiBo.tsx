...
export interface ChuyenNoiBoProps extends BaseActionProps{
    actionName : string
}
// sử dụng comp này bọc ngoài do không thể sử dụng customhook, và các hook bth ở trong useMemo, useEffect
export const ChuyenNoiBoWrapper = (props: ChuyenNoiBoProps) => {
    
    return <ChuyenNoiBo {...props}></ChuyenNoiBo>
}

const ChuyenNoiBo = ({actionName, iconName, colorCode}: ChuyenNoiBoProps) => {
    const buttonActionContext = useButtonActionContext()
    const onClick = () => {
        if(buttonActionContext.selectedHoSos.length){
            buttonActionContext.setChuyenNoiBoModalVisible(true)
        }else {
