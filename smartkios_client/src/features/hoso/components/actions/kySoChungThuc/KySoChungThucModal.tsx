...
const KySoChungThucModal = ({ setSearchHoSoParams }: ChuyenBuocXuLyModalProps) => {
    ...
    const submitHandler = async (formData: {id: string; data: ChuyenBuocXuLyHoSo}): Promise<boolean> => {
        try {
            ...
            // const dinhKem = formData.data.dinhKemKetQua
            // if (dinhKem) {
            //     let fileSigned = "";
            //     await btnSignClick(dinhKem, "KySo_" + hoSo?.maHoSo, (urlFileSigned, oldFileUrl) => {
            //         fileSigned = urlFileSigned;
            //     }, async (fileName: string) => await addPagePDF(fileName))
            //     if(fileSigned){
            //         const chuyenBuocRes = await dispatch(ChuyenBuocXuLyHoSoChungThuc({
            //             id: formData.id,
            //             data: {...formData.data, dinhKemKetQua: fileSigned}
            //         })).unwrap()
            //         if (chuyenBuocRes.succeeded) {
            //             handlerCancel();
            //         }
    ...
    return <AntdModal visible={true} keyboard={false} title={"Ký số chứng thực"} fullsizeScrollable handlerCancel={handleCancel} footer={null}>
