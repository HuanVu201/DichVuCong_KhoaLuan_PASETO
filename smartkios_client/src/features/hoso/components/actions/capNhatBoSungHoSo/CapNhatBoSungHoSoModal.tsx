...
const CapNhatBoSoHoSoModal = ({setSearchHoSoParams} : {setSearchHoSoParams : React.Dispatch<React.SetStateAction<ISearchHoSo>>}) => {
    ...
    useEffect(() => {
        (async () => {
            if(buttonActionContext.selectedHoSos.length){
                ...
                if(data){
                    setHoSo(data)
                    form.setFieldsValue({...data, hanBoSung: data.hanBoSung ? dayjs(data.hanBoSung) : undefined })
    ...
    const handleCancel = () => {
        ...
        buttonActionContext.setCapNhatBoSungHoSoModalVisible(false)
    }

    const onSave = async () => {
        const formData = await form.validateFields()
        setLoading(true)
        const res = await dispatch(MotCuaCapNhatBoSung({id: buttonActionContext.selectedHoSos[0] as string, data: formData})).unwrap()
        if(res.succeeded){
    ...
    const onSaveAndForward = async () => {
        const formData = await form.validateFields()
        setLoading(true)
        const res = await dispatch(MotCuaGuiBoSung({id: buttonActionContext.selectedHoSos[0] as string, data: formData})).unwrap()
        if(res.succeeded){
            setSearchHoSoParams((curr) => ({...curr}))
            handleCancel()
        }
    ...
    return <AntdModal title="CÁN BỘ MỘT CỬA BỔ SUNG HỒ SƠ" handlerCancel={handleCancel} visible={true} fullsizeScrollable footer={<>
        ...
        <Row>
            ...
            <Col span={12}>
                <Form.Item name="noiDungBoSung" label="Nội dung bổ sung">
                    <Input.TextArea rows={4} disabled/>
                </Form.Item>
            </Col>
            <Col span={6}>
                <Form.Item name="dinhKemBoSung" label="Đính kèm bổ sung" rules={[{required: true, message: "Vui lòng nhập nội dung"}]}>
...
export default CapNhatBoSoHoSoModal