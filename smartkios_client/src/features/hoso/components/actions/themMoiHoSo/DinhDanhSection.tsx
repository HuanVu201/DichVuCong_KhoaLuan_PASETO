...
import { FORMAT_DATE_FORMIO, getYearsFrom } from "@/data"
...
export const DinhDanhSection = ({ form }: { form: FormInstance<IHoSo> }) => {
    ...
    useEffect(() => {
        if (userCDSLDanCu !== undefined) {
            ...
            form.setFieldValue("diaChiChuHoSo", diaChi.startsWith(", ") ? diaChi.slice(2) : diaChi)
            form.setFieldValue("chuHoSo", userCDSLDanCu.fullName)
            form.setFieldValue("ngaySinhChuHoSo", userCDSLDanCu.namSinh)
            ...
            const formatedNgayThangNam = JSON.parse((userCDSLDanCu.ngayThangNamSinh as unknown as string))
            cloneUser.ngayThangNamSinh = {
                NgayThangNam : formatedNgayThangNam.NgayThangNam ? dayjs(formatedNgayThangNam.NgayThangNam).format(FORMAT_DATE_FORMIO) : undefined,
                Nam: formatedNgayThangNam.Nam
            }
    ...
    const onDinhDanh = async () => {
        ...
        if (Object.values(data).every(x => x)) {
            const res = await dispatch(GetUserFromCSDLDanCu({ 
                HoVaTen: normalizeVN2ENString(data.chuHoSo as string), 
                SoDinhDanh: data.soGiayToChuHoSo.length == 12 ? data.soGiayToChuHoSo : "", 
                Nam: data.ngaySinhChuHoSo, 
                NgayThangNam:"", 
                SoCMND: data.soGiayToChuHoSo.length == 9 ? data.soGiayToChuHoSo : "" })).unwrap()
            if(res.succeeded){
    ...
    return <Row gutter={[8, 0]}>
        ...
        <Col md={6} span={24}>
            <Form.Item name="soGiayToChuHoSo" label={(FORMAT_SOGIAYTO_LABEL as any)[loaiDoiTuong]} rules={INPUT_RULES.soGiayToChuHoSo} hasFeedback>
                <Input
                    addonAfter={<span onClick={onQuickSearch} style={{cursor:daDinhDanh ? "not-allowed": "pointer"}}><SearchOutlined/> Tìm nhanh</span>}
        ...
        {isLoaiDoiTuongCongDan ? <Col md={6} span={24}>
            <Form.Item name="ngaySinhChuHoSo" label="Năm sinh" >
                ...
                {/* <AntdSelect options={getYearsFrom1900()} showSearch placeholder="Nhập năm sinh VD:2000"disabled={daDinhDanh}></AntdSelect> */}
            </Form.Item>
        </Col>: null}
        ...
        <Col span={24}>
            ...
            </AntdButton> : isLoaiDoiTuongCongDan ? <AntdButton onClick={onDinhDanh} loading={loading} type="primary">Định danh</AntdButton> : null}
