...
const TraKetQuaHoSoModal = ({ setSearchHoSoParams }: { setSearchHoSoParams: React.Dispatch<React.SetStateAction<ISearchHoSo>> }) => {
    ...
    const handleCancel = async () => {
        ...
        dispatch(resetData())
        dispatch(resetDatas())
        buttonActionContext.setTraKetQuaModalVisible(false)
    ...
    const onOk = async () => {
        ...
        if(buttonActionContext.selectedHoSos.length){
            const res = await dispatch(TraKetQuaHoSo({...formData, id: buttonActionContext.selectedHoSos[0] as string})).unwrap()
            if(res.succeeded){
    ...
    // }
    return <AntdModal title={"TRẢ KẾT QUẢ HỒ SƠ"} visible={true} handlerCancel={handleCancel} width={1280}
        footer={<AntdSpace>
...
export default TraKetQuaHoSoModal