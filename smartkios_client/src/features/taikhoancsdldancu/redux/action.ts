...
export const SearchLogCSDLDanCuDoanhNghiep =
    createAsyncThunk<IPaginationResponse<ILogCSDLDanCuDoanhNghiep[]>, IPickSearch<ILogCSDLDanCuDoanhNghiep, "donViId" | "thoiGian" | "loai"|"taiKhoan">>("SearchLogCSDLDanCuDoanhNghiep", async (params, thunkApi) => {
        try {
...
export const StatisticLogCSDLDanCuDoanhNghiep =
    createAsyncThunk<IPaginationResponse<IStatisticLogCSDLDanCuDoanhNghiep[]>, IPickSearch<IStatisticLogCSDLDanCuDoanhNghiep, "thoiGian">>("StatisticLogCSDLDanCuDoanhNghiep", async (params, thunkApi) => {
        try {
            const res = await LogCSDLDanCuDoanhNghiepApi.Statistic(params)
            return res.data
        } catch (error) {
            return thunkApi.rejectWithValue(error)
        }
