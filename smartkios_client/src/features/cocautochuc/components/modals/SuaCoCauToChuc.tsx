...
export const SuaCoCauToChuc = ({
  ...
  const [form] = Form.useForm<Omit<ICoCauToChuc, "maNhomLienThong" | "otherCatalog"> & { maNhomLienThong?: string[], otherCatalog: string[] }>();
  const folderContext = useFolderContext();
  const dispatch = useAppDispatch();
  const onFinish: FormProps["onFinish"] = (values: Omit<ICoCauToChuc, "maNhomLienThong" | "otherCatalog"> & {
    ...
    if (folderId) {
      ...
      dispatch(UpdateCoCauToChuc({
        id: folderId,
        data:
        {
          ...values,
          // tránh lọc nhầm => khi có 1 mã thì thêm ## => H54.01##
          maNhomLienThong: maNhomLienThong ? maNhomLienThong.length == 1 ? maNhomLienThong + ID_SEPARATE_ONE_THUNK : maNhomLienThong.join(ID_SEPARATE_ONE_THUNK) : undefined,
          otherCatalog: otherCatalog ? otherCatalog.length == 1 ? otherCatalog + ID_SEPARATE_ONE_THUNK : otherCatalog.join(ID_SEPARATE_ONE_THUNK) : undefined
        }
      }));
    }
    // else {
  ...
  useEffect(() => {
    if (coCauToChuc) {
      form.setFieldsValue(
        {
          ...coCauToChuc,
          maNhomLienThong: coCauToChuc.maNhomLienThong ? coCauToChuc.maNhomLienThong.split(ID_SEPARATE_ONE_THUNK).filter(x => x != "") : undefined,
          otherCatalog: coCauToChuc.otherCatalog ? coCauToChuc.otherCatalog.split(ID_SEPARATE_ONE_THUNK).filter(x => x != "") as any : undefined,
        }
      );
    }
  }, [coCauToChuc]);
  ...
  const [groupOthersOptions, groupOptions] = useMemo(() => {
    return [danhMucChungs?.filter(e => e.type === 'nhom-co-cau-khac'), danhMucChungs?.filter(e => e.type === 'nhom-co-cau')]
  }, [danhMucChungs])
        ...
        <Row gutter={[8, 8]}>
          ...
          <Col md={12} span={24}>
            <Form.Item
              label="Nhóm"
              name="catalog">
              <AntdSelect generateOptions={{ model: groupOptions, value: "code", label: "tenDanhMuc" }} showSearch allowClear />
            </Form.Item>
          </Col>
          <Col md={12} span={24}>
            <Form.Item
              label="Mã nhóm liên thông"
              name="maNhomLienThong"
            >
              <AntdSelect mode="multiple" generateOptions={{ model: nhomLienThongs, value: "maDinhDanh", label: "groupName" }} showSearch allowClear />
            </Form.Item>
          </Col>
          <Col md={12} span={24}>
            <Form.Item
              ...
              name="otherCatalog"
            >
              <AntdSelect mode="multiple" generateOptions={{ model: groupOthersOptions, value: "code", label: "tenDanhMuc" }} showSearch allowClear />
            </Form.Item>
          </Col>
        </Row>
