...
export const ThongTinCSDLDanCu = () => {
    ...
    useEffect(() => {
        if(userCDSLDanCu != undefined){
            dataForm.setFieldsValue({
                ...
                ngayThangNamSinh: userCDSLDanCu.ngayThangNamSinh ? 
                    dayjs((JSON.parse(userCDSLDanCu.ngayThangNamSinh as unknown as string) as TSTypeHelpers.CapitalizeAllKeys<typeof userCDSLDanCu.ngayThangNamSinh>).NgayThangNam).format(FORMAT_DATE_WITHOUT_TIME) : "",
    ...
    const onFinishSearch = async (values: ThongTinCSDLDanCuSearchParams) => {
        const formData: ThongTinCSDLDanCuSearchParams = {...values, HoVaTen: normalizeVN2ENString(values.HoVaTen)}
        ...
        try {
            dispatch(GetUserFromCSDLDanCu({...formData, UpdateEntity: true}))
        } catch (error) {
            console.log(error);
        }
    }
    ...
    <Form form={searchForm} name="ThongTinCSDLDanCuSearch" layout="vertical" initialValues={{ LoaiGiayTo: "CCCD" }} onFinish={onFinishSearch}>
        <Row gutter={8} justify="center" align="middle">
            ...
            <Col md={12} span={24}>
                <Form.Item name="Nam" label="Năm sinh" rules={[{ required: true, message: "Vui lòng nhập năm sinh" }]}>
                    {/* <AntdSelect options={getYearsFrom1900()} showSearch></AntdSelect> */}
                    <AntdAutoComplete generateOptions={{model: getYearsFrom(1900), label: "label", value: "value"}} placeholder="Nhập năm sinh VD:2000">
                        <Input/>
                    </AntdAutoComplete>
                </Form.Item>
            </Col>
            ...
            <Col span={1}>
                <AntdButton type="primary" htmlType="submit">Kiểm tra</AntdButton>
            </Col>
        </Row>
    </Form>
