...
export const TruongHopThuTucSearch = ({ setSearchParams }: { setSearchParams: React.Dispatch<React.SetStateAction<ISearchTruongHopThuTuc>> }) => {
  ...
  const onFinish = (values: ISearchTruongHopThuTuc) => {
    setSearchParams({ pageNumber: 0, pageSize: 10, reFetch: true, ...values })
  }
  ...
  return (
    <CollapseContent
      extraButtons={[<AntdButton onClick={() => {TruongHopThuTucContext.setTruongHopThuTucModalVisible(true)}}>Thêm mới</AntdButton>]}
    >
