...
export const SoChungThucDetail = ({setSearchParams}: {setSearchParams: React.Dispatch<React.SetStateAction<ISearchSoChungThuc>>}) => {
    ...
    const onFinish = async () => {
        ...
        if (SoChungThucContext?.maSoChungThuc) {
            await dispatch(UpdateSoChungThuc({ id: SoChungThucContext.maSoChungThuc, data: formData })).unwrap()
        } else {
            await dispatch(AddSoChungThuc(formData)).unwrap()
        }
    ...
    useEffect(() => {
        if (SoChungThuc) {
            form.setFieldsValue({ ...SoChungThuc, tenSo: SoChungThuc.tenSo.replace("-SCTĐT/BS",""), ngayBatDau: SoChungThuc.ngayBatDau ? dayjs(SoChungThuc.ngayBatDau) : undefined, 
