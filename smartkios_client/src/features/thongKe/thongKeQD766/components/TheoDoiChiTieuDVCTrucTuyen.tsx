...
import { useTheoDoiChiTieuDVCTrucTuyenContext, TheoDoiChiTieuDVCTrucTuyenContextProvider } from "../context/ThongKeQD766Context";
import {
  SearchOutlined,
  PrinterOutlined,
  DownOutlined,
...
const items: MenuProps['items'] = [
  ...
  // },
  {
    label: <button style={{ border: 'none', background: 'inherit' }}
      onClick={() => downloadPhieuExcel('Theo dõi chỉ tiêu DVC trực tuyến các đơn vị - theo quyết định số 766/QĐ-TTG ngày 23/06/2022')}>
...
const TheoDoiChiTieuDVCTrucTuyen = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      if (thongKeHoSoContext.search.pageSize) {
        dispatch(SearchTheoDoiChiTieuDVCTrucTuyen(thongKeHoSoContext.search));
    ...
    if (item.catalog === 'so-ban-nganh') {
      ...
      dataHtmlForPdf += "<tr>" +
        ...
        ((item?.tiepNhanQuaMang / item?.tongTiepNhan) > 0
          ? Math.round((item.tiepNhanQuaMang / item.tongTiepNhan * 100) * 100) / 100 + "%"
          : ""
        ) +
        "</td>" +
        "<td style='vertical-align: middle; padding: 5px; border: 1px solid #333' >" +
        ((item?.tiepNhanTrucTiep / item?.tongTiepNhan) > 0
          ? Math.round((item.tiepNhanTrucTiep / item.tongTiepNhan * 100) * 100) / 100 + "%"
          : ""
        ) +
        "</td>" +
        "<td style='vertical-align: middle; padding: 5px; border: 1px solid #333' >" +
        ((item?.tiepNhanBCCI / item?.tongTiepNhan) > 0
          ? Math.round((item.tiepNhanBCCI / item.tongTiepNhan * 100) * 100) / 100 + "%"
          : ""
        ) +
        "</td>" +
        "<td style='vertical-align: middle; padding: 5px; border: 1px solid #333' >" +
        ...
        ((item?.trucTuyenDangXuLyTrongHan / item?.tongTrucTuyenDangXuLy) > 0
          ? Math.round((item.trucTuyenDangXuLyTrongHan / item.tongTrucTuyenDangXuLy * 100) * 100) / 100 + "%"
          : ""
        ) +
        "</td>" +
        "<td style='vertical-align: middle; padding: 5px; border: 1px solid #333' >" +
        ((item?.trucTuyenDangXuLyQuaHan / item?.tongTrucTuyenDangXuLy) > 0
          ? Math.round((item.trucTuyenDangXuLyQuaHan / item.tongTrucTuyenDangXuLy * 100) * 100) / 100 + "%"
          : ""
        ) +
        "</td>" +
        "<td style='vertical-align: middle; padding: 5px; border: 1px solid #333' >" +
  ...
  dataHtmlForExcel += "<tr>" +
    ...
    "<strong>I</strong>" +
    "</td>" +
    "<td colspan='17' style='vertical-align: middle; padding: 5px; border: 1px solid #333; text-align: left; font-size: 22px; font-family: &quot;Times New Roman&quot;, Times, serif;'>" +
    "<strong>Sở, ban, ngành</strong>" +
    "</td>" +
    "</tr>"
  ...
  dataHtmlForExcel += "<tr>" +
    ...
    "<strong>II</strong>" +
    "</td>" +
    "<td colspan='30' style='vertical-align: middle; padding: 5px; border: 1px solid #333; text-align: left; font-size: 22px; font-family: &quot;Times New Roman&quot;, Times, serif;'>" +
    "<strong>Cấp huyện (bao gồm các huyện, thị xã, thành phố và xã, phường, thị trấn)</strong>" +
    "</td>" +
    "</tr>"
    ...
    <div className="thongKeSwapper">
      ...
      <div id="ContainerSwapper" style={{ fontSize: '13px', fontFamily: "'Times New Roman', Times, serif" }}>
        <table id="table" style={{ verticalAlign: 'middle', borderCollapse: 'collapse', width: '100%', textAlign: 'center', margin: '10px 0', fontSize: '13px', fontFamily: "'Times New Roman', Times, serif" }}>
          {/* <thead> */}
          <tr>
            <td colSpan={18} style={{ verticalAlign: 'middle', padding: '5px' }}>
          ...
          <tr>
            ...
            <td colSpan={4} style={{ verticalAlign: 'middle', padding: '5px', border: '1px solid #333' }}>
              <strong>Kết quả xử lý hồ sơ nộp trực tuyến với hồ sơ đang xử lý</strong>
            </td>
            <td rowSpan={2} style={{ verticalAlign: 'middle', padding: '5px', border: '1px solid #333' }}>
      ...
      <table id="tableToExcel" style={{ display: 'none', verticalAlign: 'middle', borderCollapse: 'collapse', width: '100%', textAlign: 'center', margin: '10px 0', fontSize: '13px', fontFamily: "'Times New Roman', Times, serif" }}>
        {/* <thead> */}
        <tr>
          <td colSpan={18} style={{ verticalAlign: 'middle', padding: '5px', textAlign: 'center', fontFamily: "'Times New Roman', Times, serif", fontSize: '24px' }}>
            <strong>THEO DÕI CHỈ TIÊU DVC TRỰC TUYẾN CÁC ĐƠN VỊ - THEO QUYẾT ĐỊNH SỐ 766/QĐ-TTG NGÀY 23/06/2022</strong>
          </td>
        </tr>
        <tr>
          <td colSpan={18} style={{ verticalAlign: 'middle', padding: '5px', textAlign: 'center', fontFamily: "'Times New Roman', Times, serif", fontSize: '22px' }}>
            <strong>(Thống kê hàng tháng, quý, 6 tháng, 9 tháng, năm, hàng năm)</strong>
          </td>
        </tr>
        <tr>
