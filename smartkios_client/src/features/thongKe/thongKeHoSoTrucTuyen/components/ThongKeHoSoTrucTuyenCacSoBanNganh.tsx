...
import { IThongKeHoSoTrucTuyenCacSoBanNganhParams } from "../models/TiepNhanHoSoTrucTuyenSearch";
import { SearchTkHoSoTrucTuyenCacSoBanNganh } from "../redux/action";
import { useThongKeHoSoTrucTuyenCacSoBanNganhContext, ThongKeHoSoTrucTuyenCacSoBanNganhProvider } from "../context/ThongKeHoSoTrucTuyenContext";
import {
    SearchOutlined,
    PrinterOutlined,
    DownOutlined,
...
const items: MenuProps['items'] = [
    {
        label: <button onClick={() => { downloadPhieuWord('Bảng thống kê tiếp nhận hồ sơ trực tuyến các sở, ban, ngành tỉnh ') }} style={{ border: 'none', background: 'inherit' }}>
    ...
    // {
    //     label: <button onClick={() => downloadPhieuPdf('Bảng thống kê tiếp nhận hồ sơ trực tuyến các sở, ban, ngành tỉnh ')} style={{ border: 'none', background: 'inherit' }}>
    ...
    // },
    {
        label: <button style={{ border: 'none', background: 'inherit' }}
            onClick={() => downloadPhieuExcel('Bảng thống kê tiếp nhận hồ sơ trực tuyến các sở, ban, ngành tỉnh')}>
...
const ThongKeHoSoTrucTuyenCacSoBanNganh = () => {
    ...
    const thongKeHoSoContext = useThongKeHoSoTrucTuyenCacSoBanNganhContext();
    let [form] = Form.useForm<IThongKeHoSoTrucTuyenCacSoBanNganhParams>();
    ...
    useEffect(() => {
        (async () => {
            dispatch(SearchTkHoSoTrucTuyenCacSoBanNganh(thongKeHoSoContext.search));
        })();
    }, [thongKeHoSoContext.search]);
    ...
    thongKeDatas?.forEach((item: any, index) => {
        dataHtmlForPdf += "<tr>" +
            ...
            "</td>" +
            "<td style='vertical-align: middle; padding: 5px; border: 1px solid #333' >" +
            "<strong>" + item.tongSo + "</strong>" +
            "</td>" +
            "<td style='vertical-align: middle; padding: 5px; border: 1px solid #333' >" +
            "<strong>" + item.tongTrucTuyen + "</strong>" +
            "</td>" +
            "<td style='vertical-align: middle; padding: 5px; border: 1px solid #333' >" +
            "<strong>" + item.tongTrucTiep + "</strong>" +
            "</td>" +
            "<td style='vertical-align: middle; padding: 5px; border: 1px solid #333' >" +
            "<strong>" + item.tongBCCI + "</strong>" +
            "</td>" +
            "<td rowspan='3' style='vertical-align: middle; padding: 5px; border: 1px solid #333' >" +
            ((item.tongTrucTuyen / item.tongSo) > 0
                ? "<strong>" + Math.round((item.tongTrucTuyen / item.tongSo * 100) * 100) / 100 + "%</strong>"
                : "<strong></strong>"
            )
            + "</td>" +
            "<td rowspan='3' style='vertical-align: middle; padding: 5px; border: 1px solid #333' >" +
            ...
            ((item.tongToanTrinhTrucTuyen / item.tongToanTrinh) > 0
                ? "<strong>" + Math.round((item.tongToanTrinhTrucTuyen / item.tongToanTrinh * 100) * 100) / 100 + "%</strong>"
                : "<strong></strong>"
            )
            + "</td>" +
            "<td rowspan='3' style='vertical-align: middle; padding: 5px; border: 1px solid #333' >" +
    ...
    return (
        <div className="thongKeSwapper">
            <div className="headerThongKe">
                <div className="title">
                    <h6>BẢNG THỐNG KÊ TIẾP NHẬN HỒ SƠ TRỰC TUYẾN CỦA CÁC SỞ, BAN, NGÀNH CẤP TỈNH</h6>
                </div>
            ...
            <div id="ContainerSwapper">
                ...
                <table id="table" style={{ verticalAlign: 'middle', borderCollapse: 'collapse', width: '100%', textAlign: 'center', margin: '10px 0', fontSize: '17px', fontFamily: "'Times New Roman', Times, serif" }}>
                    {/* <thead> */}
                    <tr>
                        <td colSpan={11} style={{ verticalAlign: 'middle', padding: '5px', fontSize: '19px' }}>
                            <strong>PHỤ LỤC 2</strong><br />
                            <strong>BẢNG THỐNG KÊ TIẾP NHẬN HỒ SƠ TRỰC TUYẾN CỦA CÁC SỞ, BAN, NGÀNH CẤP TỈNH </strong><br />
                    ...
                    <tr>
                        ...
                        <td rowSpan={3} style={{ verticalAlign: 'middle', padding: '5px', border: '1px solid #333', width: '25%' }}>
                        ...
                        <td rowSpan={2} colSpan={4} style={{ verticalAlign: 'middle', padding: '5px', border: '1px solid #333' }}>
                            ...
                            <span>(Tỷ lệ nộp hồ sơ trực tuyến: 90%; Tỷ lệ nộp hồ sơ trực tuyến toàn trình: 85%)</span>
                        </td>
                    ...
                    <tr>
                        <td rowSpan={2} style={{ verticalAlign: 'middle', padding: '5px', border: '1px solid #333', width: '8%' }}>
                    ...
                    <tr>
                        <td style={{ verticalAlign: 'middle', padding: '5px', border: '1px solid #333', width: '8%' }}>
            ...
            <table id="tableToExcel" style={{ display: 'none', verticalAlign: 'middle', borderCollapse: 'collapse', width: '100%', textAlign: 'center', margin: '10px 0', fontSize: '13px', fontFamily: "'Times New Roman', Times, serif" }}>
                <thead>
                    <tr>
                        <td colSpan={11} style={{ verticalAlign: 'middle', padding: '5px', textAlign: 'center', fontFamily: "'Times New Roman', Times, serif", fontSize: '24px' }}>
                            <strong>PHỤ LỤC 2</strong><br />
                            <strong>BẢNG THỐNG KÊ TIẾP NHẬN HỒ SƠ TRỰC TUYẾN CỦA CÁC SỞ, BAN, NGÀNH CẤP TỈNH </strong><br />
                            <strong className="filterDate" dangerouslySetInnerHTML={{ __html: dateToDate || " " }} />
                        </td>
...
function ThongKeHoSoTrucTuyenCacSoBanNganhSwapper() {
    return (
        <ThongKeHoSoTrucTuyenCacSoBanNganhProvider>
            <ThongKeHoSoTrucTuyenCacSoBanNganh />
        </ThongKeHoSoTrucTuyenCacSoBanNganhProvider>
    );
}
export default ThongKeHoSoTrucTuyenCacSoBanNganhSwapper
