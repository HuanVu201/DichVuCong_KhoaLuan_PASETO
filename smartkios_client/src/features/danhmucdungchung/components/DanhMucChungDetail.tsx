...
export const DanhMucChungDetail = () => {
    ...
    const onFinish = async () => {
        ...
        if (danhMucChungContext?.danhMucChungId) {
            dispatch(UpdateDanhMucChung({ id: danhMucChungContext.danhMucChungId, data: { ...formData, type: searchRouterParams.get("type") || "" } },))
        } else {
            dispatch(AddDanhMucChung({ ...formData, type: searchRouterParams.get("type") || "" }))
        }
        handleCancel()
    }
    const handleCancel = () => {
