    ...
    <div className="rounded overflow-hidden border">
      ...
      <div className="px-2 py-1">
        {thuTucNoiBats?.map((thuTuc, index) => {
          return <CardThuTucPortalNoiBat key={index} thuTuc={thuTuc} />;
        })}
      </div>
    </div>
  );
...
const CardThuTucPortalNoiBat = ({thuTuc}:{thuTuc: IThuTuc} ) => {
  ...
  // }, [thuTuc,searchParams])
  return (
    <Link className="text-dark" to={Service.primaryRoutes.portaldvc.nopHoSoTrucTuyen + "?maTTHC=" + thuTuc.maTTHC }>
