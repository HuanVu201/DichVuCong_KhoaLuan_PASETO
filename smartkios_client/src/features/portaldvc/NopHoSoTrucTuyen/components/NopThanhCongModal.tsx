...
export const NopThanhCongModal = ({hoSo, handlerClose}: {hoSo: Pick<IHoSo, "chuHoSo" | "maHoSo" | "thoiGianThucHien" | "tenTTHC" | "groupName"> | undefined, handlerClose: () => void}) => {
    ...
    return <Modal keyboard={false} open={true} maskClosable={false} footer={null} onCancel={handlerClose} wrapClassName="modal-saved-document" width={760} mask={false} closable={false}>
        <h5 className="text-center" style={{color: '#b71a1f', fontWeight:800 }}>Đăng ký hồ sơ thành công</h5>
        ...
        <div className="d-flex align-items-center justify-content-around mt-5">
            <button className="btn btn-yellow btn-md" role="link" onClick={() => navigate(Service.primaryRoutes.portaldvc.hosocanhan.dichVuCongCuaToi)}>
                Quản lý hồ sơ cá nhân
            </button>
            <button className="btn btn-yellow btn-md" role="link" onClick={() => navigate(Service.primaryRoutes.portaldvc.dvcTrucTuyen)}>
                Đăng ký hồ sơ khác
            </button>
            <button className="btn btn-yellow btn-md" role="link" onClick={() => navigate(Service.primaryRoutes.portaldvc.home)}>
                Về trang chủ
            </button>
        </div>
    </Modal>
