...
export const ThongTinHoSo = ({form, setDiaBan, diaBan, hideRule}: {hideRule: boolean | undefined; form: FormInstance<IHoSo>; diaBan: DiaBanChuHoSo; setDiaBan: React.Dispatch<React.SetStateAction<DiaBanChuHoSo>> }) => {
    ...
    return (
        <Row gutter={8}>
            ...
            <Col span={24} lg={6} md={12}>
                <Form.Item name="soGiayToChuHoSo" label="Số giấy tờ (CMND/CCCD/MST):" rules={hideRule ? undefined : [{ required: true, message: "Vui lòng nhập thông tin" }]}>
                    <Input  disabled={uyQuyen} />
                </Form.Item>
            </Col>
            <Col span={24} lg={6} md={12}>
                <Form.Item name="soDienThoaiChuHoSo" label="Số điện thoại:" rules={hideRule ? undefined : [{ required: true, message: "Vui lòng nhập số điện thoại" }]}>
                    <Input />
                </Form.Item>
            </Col>
            <Col span={24} lg={6} md={12}>
            ...
            <Col span={24} lg={6} md={12}>
                <Form.Item name="tinhThanhChuHoSo" label="Tỉnh/thành:" rules={hideRule ? undefined : [{ required: true, message: "Vui lòng nhập thông tin" }]}>
                    <AntdSelect disabled={uyQuyen}  generateOptions={{model: diaBan.maTinh, value: "maDiaBan" , label: "tenDiaBan"}} showSearch allowClear  onChange={(value) => onChangeMaDonVi(value, "Huyen", "Tinh")}/>
                </Form.Item>
            </Col>
            <Col span={24} lg={6} md={12}>
