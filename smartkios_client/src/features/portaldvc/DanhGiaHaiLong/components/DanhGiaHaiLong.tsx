...
export const DanhGiaHaiLongPortal = () => {
    ...
    useEffect(() => {
        if (hoso && maHoSoNguoiDungNhap) {
            if (hoso?.trangThaiHoSoId != '9' && hoso?.trangThaiHoSoId != '10') {
                ...
                buttonActionContext.setDanhGiaHaiLongModalVisible(true)
                form.resetFields()
            }
        }
    }, [hoso])
    ...
    const [tenDonVi] = useMemo(() => {
        return [publicModule?.find(x => x.code == 'ten-don-vi')]
    }, [publicModule])
    ...
    return (
        <>
            ...
            <div style={{ marginTop: '80px' }}>
                <div className="thanhToan-title" style={{ marginBottom: '20px' }}>
                    <div className="text-center" style={{ fontSize: '22px', fontStyle: 'normal', fontWeight: 'bold', color: '#cd171e', marginBottom: '5px' }}>
                        ĐÁNH GIÁ CHẤT LƯỢNG DỊCH VỤ HÀNH CHÍNH CÔNG
                    </div>
                    <p style={{ fontWeight: '600' }} className="text-center">Xin vui lòng nhập mã số hồ sơ người nộp hồ sơ để tiếp tục đánh giá</p>
                </div>
                <Form form={form} name="FilterDanhGiaHaiLong" onFinish={onFinish}>
                    <div className="d-flex justify-content-center align-items-center flex-column" >
                        <div className="col-xs-12 col-sm-5 col-sm-offset-1 form-left" >
                            <div >
                                ...
                                <div className="thanhToan-footer d-flex justify-content-center align-items-center">
                                    <AntdButton
                                        style={{ color: 'white', backgroundColor: '#0066b3', fontWeight: '600' }}
                                        htmlType="submit"
                                    >
                                        Thực hiện
                                    </AntdButton>
                        ...
                        <div className="col-md-12 col-sm-12 col-xs-12 text-center" style={{ marginTop: '30px', width: '80%', fontSize: '15px', fontStyle: 'normal', fontWeight: '600', color: '#cd171e' }}>
                            <span >
                                Để nâng cao chất lượng phục vụ của cơ quan hành chính nhà nước và của cán bộ, công chức, viên chức trong giải quyết thủ tục hành chính, kính mong Quý Ông/Bà dành thời gian để đánh giá sự hài lòng về dịch vụ hành chính công mà Quý Ông/Bà đã hoặc đang thực hiện.
                            </span>
                        </div>
            ...
            {buttonActionContext.danhGiaHaiLongModalVisible ? <DanhGiaHaiLongPortalModal id={hoso?.id} donViId={hoso?.donViId as any} maHoSoNguoiDungNhap={maHoSoNguoiDungNhap}></DanhGiaHaiLongPortalModal> : <></>}
        </>
    )
}
