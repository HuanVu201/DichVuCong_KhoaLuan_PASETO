...
import { DaChuyenCoKetQuaProvider, useDaChuyenCoKetQuaContext } from "../contexts/DaChuyenCoKetQuaContext"
...
const DaChuyenCoKetQuaTable = () => {
    ...
    const {btnElememts} = useButtonActions({maScreen: screenType["da-chuyen-co-ket-qua"]})
    const [searchParams, setSearchParams] = useState<ISearchHoSo>({ pageNumber: 1, pageSize: 10, reFetch: true, nguoiDaXuLy: user?.id, inMaTrangThais: ["9", "10"]})
    ...
    return (
        <LazyActions setSearchParams={setSearchParams}>
            <AntdSpace direction="vertical" style={{width:"100%"}}>
                ...
                {searchParams.nguoiDaXuLy ? <AntdTable
                    columns={columns}
                    dataSource={hoSos}
                    pagination={{
                        total: count
...
const HoSoTableWrapper = () => (<DaChuyenCoKetQuaProvider>
