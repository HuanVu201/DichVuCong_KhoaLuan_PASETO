...
import { resetDatas as resetLinhVucs } from "@/features/linhvuc/redux/slice";
import { SearchCoCauToChuc } from "@/features/cocautochuc/redux/crud";
import { resetDatas as resetGroups} from "@/features/cocautochuc/redux/slice";

export const HoSoLienThongSearch = ({
  setSearchParams,
}: {
  ...
  useEffect(() => {
    if (user) {
      ...
      dispatch(SearchCoCauToChuc({cataLog: "so-ban-nganh", pageNumber: 1, pageSize:100, reFetch: true}))
    }
  }, [user]);

  useEffect(() => {
    return () => {
        dispatch(resetLinhVucs())
    ...
    <CollapseContent defaultVisible>
      <Form name="HoSoSearch" layout="vertical" onFinish={onFinish} form={form}>
        <Row gutter={[8, 0]}>
          ...
          <Col md={12} span={24}>
            <Form.Item label="Sở ban ngành" name="groupCode">
              <AntdSelect
                generateOptions={{
                  model: groups,
                  label: "groupName",
