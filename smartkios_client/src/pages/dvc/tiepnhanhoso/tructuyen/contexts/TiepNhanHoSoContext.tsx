...
const TiepNhanHoSoTrucTuyenContext = createContext<ITiepNhanHoSoTrucTuyenContext | null>(null)

export interface ITiepNhanHoSoTrucTuyenContext {
...
export const useTiepNhanHoSoTrucTuyenContext = () => {
    const context = useContext(TiepNhanHoSoTrucTuyenContext)
    if (!context)
        throw new Error("TiepNhanHoSoContext must be used inside TiepNhanHoSoContext.Provider")
    return context
}

export const TiepNhanHoSoTrucTuyenProvider = ({ children }: IWithChildren) => {
    ...
    const [soHoaData, setSoHoaData] = useState<ITiepNhanHoSoTrucTuyenContext["soHoaData"]>()
    const [khoSoHoaData, setKhoSoHoaData] = useState<{onOk: (dinhKem:string, maGiayToKhoQuocGia: string) => void}>()
    // thêm các hàm search cho các tabs ở đây
    return <TiepNhanHoSoTrucTuyenContext.Provider value={{
        ...
        {children}
    </TiepNhanHoSoTrucTuyenContext.Provider>
}