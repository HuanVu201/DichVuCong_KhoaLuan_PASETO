...
public class SearchTheoThongKeTongHopDonViHandler : IRequestHandler<SearchTheoThongKeTongHopDonViRequest, PaginationResponse<HoSoDto>>
{
    ...
    public SearchTheoThongKeTongHopDonViHandler(IDapperRepository dapperRepository, ICommonServices commonServices)
    {
    ...
    public async Task<PaginationResponse<HoSoDto>> Handle(SearchTheoThongKeTongHopDonViRequest request, CancellationToken cancellationToken)
    {
        ...
        if (request.LoaiDuLieuKetNois != null && request.LoaiDuLieuKetNois.Count > 0) where += $" AND ({hoSoTableName}.LoaiDuLieuKetNoi IN @LoaiDuLieuKetNois)";
        if (request.TrangThaiHoSoIds != null && request.TrangThaiHoSoIds.Count > 0) where += $" AND ({hoSoTableName}.TrangThaiHoSoId IN @TrangThaiHoSoIds)";
