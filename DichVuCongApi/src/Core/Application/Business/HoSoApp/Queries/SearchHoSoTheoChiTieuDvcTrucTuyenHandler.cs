...
public class SearchHoSoTheoChiTieuDvcTrucTuyenHandler : IRequestHandler<SearchHoSoTheoChiTieuDvcTrucTuyen, PaginationResponse<HoSoDto>>
{
    ...
    public SearchHoSoTheoChiTieuDvcTrucTuyenHandler(IDapperRepository dapperRepository)
    {
    ...
    public async Task<PaginationResponse<HoSoDto>> Handle(SearchHoSoTheoChiTieuDvcTrucTuyen request, CancellationToken cancellationToken)
    {
        ...
        var builder = new QuyetDinh766ChiTieuDVCTrucTuyenWhereBuilder(hoSoTableName);
        ...
        string sql = $"SELECT {hoSoTableName}.ID, {hoSoTableName}.ChuHoSo, {hoSoTableName}.SoDienThoaiChuHoSo, {hoSoTableName}.EmailChuHoSo, {hoSoTableName}.MaTruongHop, {hoSoTableName}.MaTTHC, {hoSoTableName}.TrangThaiHoSoId," +
            ...
            $"INNER JOIN {donViThuTucTableName} ON {donViThuTucTableName}.MaTTHC = {hoSoTableName}.MaTTHC AND {donViThuTucTableName}.DonViId = {hoSoTableName}.DonViId " +
            $"INNER JOIN {groupTableName} " +
            $"ON {hoSoTableName}.DonViId = {groupTableName}.GroupCode " +
            $"WHERE  {hoSoTableName}.DeletedOn IS NULL AND {donViThuTucTableName}.DeletedOn IS NULL  AND {thuTucTableName}.DeletedOn IS NULL AND {groupTableName}.DeletedOn IS NULL " +
            $"AND {hoSoTableName}.TrangThaiHoSoId IN ({builder.TRANG_THAI_TIEP_NHAN}) " +
            $" {where}";
