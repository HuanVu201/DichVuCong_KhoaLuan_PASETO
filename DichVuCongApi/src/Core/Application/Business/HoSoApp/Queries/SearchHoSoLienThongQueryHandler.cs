...
public class SearchHoSLienThongQueryWhereBuilder
{
    public static string Build(SearchHoSoLienThongQuery req)
    {
        YeuCauThanhToanConstants yeuCauThanhToanConstants = new YeuCauThanhToanConstants();
        string where = "ChoBanHanh = 1"; // ChoBanHanh = 1 là hồ sơ đang ở trên QLVB
...
public class SearchHoSoLienThongQueryHandler : IRequestHandler<SearchHoSoLienThongQuery, PaginationResponse<HoSoLienThongDto>>
{
    ...
    public SearchHoSoLienThongQueryHandler(IDapperRepository dapperRepository, IUserService currentUser)
    {
        _dapperRepository = dapperRepository;
        _currentUser = currentUser;
    }
    public async Task<PaginationResponse<HoSoLienThongDto>> Handle(SearchHoSoLienThongQuery request, CancellationToken cancellationToken)
    {
        ...
        var where = SearchHoSLienThongQueryWhereBuilder.Build(request);
        var sql = $@"SELECT hs.ID,hs.NgayXacNhanKetQua, ChuHoSo,hs.TenDiaBan,hs.TenBuocHienTai ,hs.SoKyHieuKetQua,SoDienThoaiChuHoSo, EmailChuHoSo, hs.MaTruongHop, hs.MaTTHC, hs.TrangThaiHoSoId,LyDoTuChoi,LyDoBoSung,hs.TrichYeuHoSo, hs.ViTriDeHoSo,hs.LoaiKetQua,
		            UyQuyen, hs.NguoiUyQuyen, hs.SoDienThoaiNguoiUyQuyen, NgayTiepNhan, NgayHenTra, hs.CreatedOn, hs.LastModifiedOn, hs.MaHoSo,hs.DonViId, KenhThucHien,thtt.BatBuocDinhKemKetQua, thtt.Ten as TenTruongHop,thtt.BatBuocKySoKetQua, hs.DinhKemKetQua, hs.TrichYeuKetQua
                    , tt.TenTTHC,tt.TrangThaiPhiLePhi ,tt.ThuTucKhongCoKetQua,g.GroupName as TenDonVi, NgayNopHoSo, hs.SoGiayToChuHoSo,
                    hs.DangKyNhanHoSoQuaBCCIData,hs.NgayDangKyBuuDien,hs.NgayTraBuuDien,hs.TrangThaiTraBuuDien, TrangThaiThuPhi,hs.NgayHenTraCaNhan,kqlq.LoaiKetQuaHSLT,kqlq.TrichyeuHSLT,kqlq.DinhKemHSLT
                    ...
                    outer apply (
                       SELECT  
                        STRING_AGG(k.LoaiKetQua, ', ') AS LoaiKetQuaHSLT, 
                        STRING_AGG(k.SoKyHieu, ', ') AS SoKyHieuHSLT,
                        STRING_AGG(k.TrichYeu, ', ') AS TrichyeuHSLT,
                        STRING_AGG (CONVERT(NVARCHAR(700), DinhKem) , '##') as DinhKemHSLT
                        FROM  Business.KetQuaLienQuans k where hs.MaHoSo = k.MaHoSo
                        ) as kqlq
                    {where}";
        string order = string.Empty;

        var lstOrderCol = request.OrderBy != null && request.OrderBy.Length > 0 ? request.OrderBy.ToList() : new List<string>() { "CreatedOn Desc" };
        foreach (var item in lstOrderCol.Select((value, i) => new { i, value }))
        {

            order += item.i == lstOrderCol.Count - 1 ? $" {Regex.Replace(item.value, @"[^0-9a-zA-Z]+", " ")}" : $" {Regex.Replace(item.value, @"[^0-9a-zA-Z]+", " ")},";
        }
        var data = await _dapperRepository.PaginatedListSingleQueryAsync<HoSoLienThongDto>(sql, request.PageSize, order, cancellationToken, request.PageNumber, new
        {
            request.MaHoSo,
