...
{
    public static string Build(SearchThuTucQuery req)
    {
...
public class SearchThuTucQueryHandler : IRequestHandler<SearchThuTucQuery, PaginationResponse<ThuTucDto>>
{
    ...
    public SearchThuTucQueryHandler(IDapperRepository dapperRepository, IUserService userService)
    {
    ...
    public async Task<PaginationResponse<ThuTucDto>> Handle(SearchThuTucQuery request, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrEmpty(request.DonVi) || request.currentDonVi == true)
        {
            searchByGroupCode = @$"INNER JOIN (
	                select distinct dvtt.MaTTHC, dvtt.NguoiTiepNhanId from Catalog.DonViThuTucs dvtt where
