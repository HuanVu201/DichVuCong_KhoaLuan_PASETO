...
public class SearchLinhVucTheoBaoCaoTongHopQueryHandler : IRequestHandler<SearchLinhVucTheoBaoCaoTongHopQuery, PaginationResponse<LinhVucDto>>
{
    ...
    public SearchLinhVucTheoBaoCaoTongHopQueryHandler(IDapperRepository dapperRepository)
    {
        _dapperRepository = dapperRepository;
        _tiepNhanHoSoTrucTuyenConstants = new TiepNhanHoSoTrucTuyenConstants();
    }

    public async Task<PaginationResponse<LinhVucDto>> Handle(SearchLinhVucTheoBaoCaoTongHopQuery request, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrEmpty(request.Catalog) && string.IsNullOrEmpty(request.MaDinhDanh) && string.IsNullOrEmpty(request.MaDinhDanhCha))
        {
            ...
            {
                where += $" AND {groupsTableName}.DonViQuanLy = @Catalog ";
            }
            else
            {
        ...
        var sql = $"SELECT DISTINCT {linhVucTableName}.ID, {linhVucTableName}.Ten, {linhVucTableName}.Ma, {linhVucTableName}.MaNganh, " +
            ...
            $"ON {linhVucTableName}.Ma = {thuTucTableName}.MaLinhVucChinh ";
