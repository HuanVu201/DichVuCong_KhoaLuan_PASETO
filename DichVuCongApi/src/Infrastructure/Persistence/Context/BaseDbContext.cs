...
using Dapper;
using TD.LienThongDichVuCong.Application.Common.Persistence;
...
public abstract class BaseDbContext : MultiTenantIdentityDbContext<ApplicationUser, ApplicationRole, string, IdentityUserClaim<string>, IdentityUserRole<string>, IdentityUserLogin<string>, ApplicationRoleClaim, IdentityUserToken<string>>
{
    ...
    private List<AuditTrail> HandleAuditingBeforeSaveChanges(Guid userId)
    {
            ...
            .ToList())
        {
            ...
            var listTbl = new List<string>() { "User", "Group", "Menu", "NhomNguoiDung", "NguoiDungNhomNguoiDung", "SoChungThuc", "ThuTuc", "TruongHopThuTuc", "ThanhPhanThuTuc", "QuyTrinhXuLy", "Screen", "ScreenAction", "Action", "LinhVuc", "MauPhoi", "DonViThuTuc" };
            if (!listTbl.Contains(entry.Entity.GetType().Name))
                continue;
