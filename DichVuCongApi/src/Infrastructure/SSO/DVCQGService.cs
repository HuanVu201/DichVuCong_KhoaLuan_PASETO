...
public class DVCQGService : IDVCQGService
{
    private IDapperRepository _dapperRepository;
    private UserManager<ApplicationUser> _userManager;
    private JwtSettings _jwtSettings;
    private TDTenantInfo? _currentTenant;
    private IConfiguration _configuration;
    private IRepositoryWithEvents<LogCSDLDanCuDoanhNghiep> _repositoryLogCSDL;
    private IServiceLogger _serviceLogger;
    private ILogger<DVCQGService> _logger;

    public DVCQGService(IDapperRepository dapperRepository, UserManager<ApplicationUser> userManager, IOptions<JwtSettings> jwtSettings, TDTenantInfo? currentTenant, IConfiguration configuration, IRepositoryWithEvents<LogCSDLDanCuDoanhNghiep> repositoryLogCSDL, IServiceLogger serviceLogger, ILogger<DVCQGService> logger)
    {
    ...
    public async Task<string?> KiemTraNguoiDungDVCQuocGia(UserInfoDvcqgRequest request)
    {
        ...
        try
        {
            if (request != null)
            {
                ...
                string? maSoThue = request.MaSoThue != null ? request.MaSoThue : request.MaSoDoanhNghiep;
                    ...
                    if (userCheck != null)
                    {
                        ...
                        if (userCheck.TypeUser == "CanBo")
                        {
                            if (userCheck?.DaXacThucCSDLDC == null || userCheck?.DaXacThucCSDLDC == false)
                            {
                                ...
                                userCheck.SoCMND = request.SoCMND;
                ...
                else if (request.LoaiTaiKhoan == "2" && !string.IsNullOrEmpty(maSoThue))
                {
                    var userCheck = _userManager.Users.Where(x => x.UserName == maSoThue).FirstOrDefault();
                    if (userCheck != null)
                    {
                        userCheck.IdTokenDVCQG = request.id_token;
                        userCheck.AccessTokenDVCQG = request.access_token;
                    ...
                    else
                    {
                        userCheck = new ApplicationUser()
                        {
                            ...
                            SoDinhDanh = maSoThue,
                            Email = request.ThuDienTu ?? string.Concat(soGiayTo, "@dichvucong.gov.vn"),
                            FullName = request.TenDoanhNghiep.Length <= 150 ? request.TenDoanhNghiep : request.TenDoanhNghiep.Substring(150),
                            NguoiDaiDien = request.NguoiDaiDien,
                            ThuongTru = request.DiaChi,
                            UserOrder = 100,
                            TypeUser = "CongDan",
                            IdTokenDVCQG = request.id_token,
                            AccessTokenDVCQG = request.access_token,
                ...
                else if (request.LoaiTaiKhoan == "3") // Tài khoản tổ chức
                {
                    var userCheck = _userManager.Users.Where(x => x.UserName == request.TechID).FirstOrDefault();
                    if (userCheck != null)
                    {
                        userCheck.IdTokenDVCQG = request.id_token;
                        userCheck.AccessTokenDVCQG = request.access_token;
        ...
        {
            _logger.LogInformation("KiemTraNguoiDungDVCQuocGia_error" +
    ...
    public async Task<string?> GetTokenAsync(string accessToken, string? ipAddress, string? device = null)
    {
        var user = _userManager.Users.FirstOrDefault(x => x.AccessTokenDVCQG == accessToken);
        if (user != null)
        {
